...
Процедура ДеревоЦенНоменклатураПартнераАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	МассивНоменклатурыКонтрагентов = НоменклатураПартнераАвтоПодборПолучитьМассивПоТексту(Текст, Объект.Партнер,
										ТекущиеДанные.Номенклатура, ТекущиеДанные.Характеристика, Упаковка);
...
&НаСервереБезКонтекста
Функция НоменклатураПартнераАвтоПодборПолучитьМассивПоТексту(Текст, Партнер, Номенклатура, Характеристика, Упаковка)
	...
	|	Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.НоменклатураКонтрагентов
	|ГДЕ
	|	ВладелецНоменклатуры = &Партнер
	|	И НЕ ПометкаУдаления
	|	И НЕ Недействителен
	|	И Наименование ПОДОБНО &Текст + ""%""
	|	//%УсловиеОтбора%
	|";
	...
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		МассивОтборов.Добавить("Номенклатура = &Номенклатура");
		Запрос.УстановитьПараметр("Номенклатура", Номенклатура);
	КонецЕсли;

	Если ЗначениеЗаполнено(Характеристика) Тогда
		МассивОтборов.Добавить("Характеристика = &Характеристика");
		Запрос.УстановитьПараметр("Характеристика", Характеристика);
	КонецЕсли;

	Если ЗначениеЗаполнено(Упаковка) Тогда
		МассивОтборов.Добавить("Упаковка = &Упаковка");
		Запрос.УстановитьПараметр("Упаковка", Упаковка);
	КонецЕсли;

	Если ЗначениеЗаполнено(МассивОтборов) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//%УсловиеОтбора%", "И " + СтрСоединить(МассивОтборов, " И "));
	КонецЕсли;
