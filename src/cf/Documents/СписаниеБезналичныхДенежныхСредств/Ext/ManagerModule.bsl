...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = '- заполнение реквизитов ""Сумма в валюте списания"" и ""Сумма НДС в валюте списания"" расшифровки платежа';"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.СписаниеБезналичныхДенежныхСредств КАК ДанныеДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ДанныеДокументаРасшифровкаПлатежа
	|		ПО ДанныеДокумента.Ссылка = ДанныеДокументаРасшифровкаПлатежа.Ссылка
	|			И ДанныеДокумента.ПлатежСКонвертацией
	|			И ДанныеДокументаРасшифровкаПлатежа.СуммаВВалютеОтправителя = 0
	|			И ДанныеДокументаРасшифровкаПлатежа.Сумма <> 0
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = 
		...
		|ВЫБРАТЬ
		|	ТаблицаДокументов.Ссылка КАК Ссылка,
		|	ДанныеРасшифровкиПлатежа.ИдентификаторСтроки КАК ИдентификаторСтроки,
		|	СуммыДокументовВВалютахУчета.СуммаБезНДС + СуммыДокументовВВалютахУчета.СуммаНДС КАК Сумма,
		...
		|			И ДанныеДокумента.ПлатежСКонвертацией
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ДанныеРасшифровкиПлатежа
		|		ПО ДанныеРасшифровкиПлатежа.Ссылка = ДанныеДокумента.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыДокументовВВалютахУчета
		|		ПО ДанныеРасшифровкиПлатежа.Ссылка = СуммыДокументовВВалютахУчета.Регистратор
		|			И ДанныеРасшифровкиПлатежа.ИдентификаторСтроки = СуммыДокументовВВалютахУчета.ИдентификаторСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	БанковскиеСчета.ВалютаДенежныхСредств КАК ВалютаПлательщика
		|ИЗ
		|	ТаблицаДокументов КАК ТаблицаДокументов
		...
		|		ПО (ДанныеДокумента.БанковскийСчет = БанковскиеСчета.Ссылка)";
	
	Запрос.УстановитьПараметр("ДанныеДляОбновления", ОбновляемыеДанные);
	
	МассивЗапросов = Запрос.ВыполнитьПакет();
	ВыборкаСуммДокументов = МассивЗапросов[МассивЗапросов.ВГраница() - 1].Выбрать();
	Документ = МассивЗапросов[МассивЗапросов.ВГраница()].Выбрать();
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ДокументОбъект.ПлатежСКонвертацией Тогда
					...
					Для Каждого СтрокаРасшифровки Из ДокументОбъект.РасшифровкаПлатежа Цикл
						...
						Если СтрокаРасшифровки.СуммаВВалютеОтправителя = 0
							И СтрокаРасшифровки.Сумма <> 0 Тогда
							
							Отбор = Новый Структура("Ссылка, ИдентификаторСтроки");
							Отбор.Ссылка = Документ.Ссылка;
							Отбор.ИдентификаторСтроки = СтрокаРасшифровки.ИдентификаторСтроки;
							
							Если ВыборкаСуммДокументов.НайтиСледующий(Отбор) Тогда
								
								СтрокаРасшифровки.СуммаВВалютеОтправителя = ВыборкаСуммДокументов.Сумма;
								СтрокаРасшифровки.СуммаНДСВВалютеОтправителя = ВыборкаСуммДокументов.СуммаНДС;
					...
					КоэффициентПересчета = РаботаСКурсамиВалютУТ.ПолучитьКоэффициентПересчетаИзВалютыВВалюту(
						...
						Документ.ВалютаПлательщика,
						?(ДокументОбъект.ПроведеноБанком, ДокументОбъект.ДатаПроведенияБанком, ДокументОбъект.Дата));
					
					СуммаПересчитанная = Окр(КоэффициентПересчета * ДокументОбъект.РасшифровкаПлатежа.Итог("Сумма"), 2);
					СуммаНДСПересчитанная = Окр(КоэффициентПересчета * ДокументОбъект.РасшифровкаПлатежа.Итог("СуммаНДС"), 2);
					
					Если СуммаПересчитанная <> ДокументОбъект.РасшифровкаПлатежа.Итог("СуммаВВалютеОтправителя")
						Или СуммаНДСПересчитанная <> ДокументОбъект.РасшифровкаПлатежа.Итог("СуммаНДСВВалютеОтправителя") Тогда 
						ДенежныеСредстваСервер.РассчитатьСуммыВВалютеОтправителяВРасшифровке(
							ДокументОбъект, Документ.ВалютаПлательщика, КоэффициентПересчета);
					КонецЕсли;
