...
Функция ТекстЗапросаОстатковПриСменеНалогообложения()
	...
	|				&ГраницаКонецПериода,
	|				ВидДеятельностиНДС В (&ИсходныеВидыДеятельности)
	|				И Организация = &Организация И РазделУчета В (&РазделыУчета)
	|		) КАК Остатки
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	...
	|	ГДЕ
	|		Остатки.КоличествоОстаток > 0 И ЕСТЬNULL(Назначения.ВидДеятельностиНДС, НЕОПРЕДЕЛЕНО) <> Остатки.ВидДеятельностиНДС
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		Остатки.Регистратор = &Ссылка И ЛОЖЬ
	|		И Остатки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|) КАК тПартии
	|
	|СГРУППИРОВАТЬ ПО
