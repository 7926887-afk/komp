...
Функция ИсходящаяНакладнаяJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	|	ПРЕДСТАВЛЕНИЕ(Товары.Партия)                КАК ПредставлениеПартии,
	|	ЕСТЬNULL(Товары.Партия.УпаковочнаяЕдиница.КоличествоСАТУРН, 0) КАК КоличествоВУпаковкеПартии,
	...
	ОшибкиУпаковок = ИнтеграцияСАТУРНСлужебный.ОшибкиКоличествоВУпаковкеНеСоответствуетКоличествуВУпаковкеПартии(ВсеТовары);
	Если ОшибкиУпаковок.Количество() > 0 Тогда
		АбстрактноеСообщениеJSON.ТекстОшибки = СтрСоединить(ОшибкиУпаковок, Символы.ПС);
		СообщенияJSON.Добавить(АбстрактноеСообщениеJSON);
		Возврат СообщенияJSON;
	КонецЕсли;
...
Функция СообщениеИсходящейНакладной(Шапка, Товары, АбстрактноеСообщениеJSON)
	...
	Для Каждого ЭлементКоллекции Из Товары Цикл
		...
		ЭлементДанных.Вставить("countPuSent",      ЭлементКоллекции.КоличествоУпаковок);
...
Функция СообщениеИсходящейНакладнойЭкспорт(Шапка, Товары, АбстрактноеСообщениеJSON)
	...
	Для Каждого ЭлементКоллекции Из Товары Цикл
		...
		ЭлементДанных = Новый Структура;
		ЭлементДанных.Вставить("id",                0);
		ЭлементДанных.Вставить("countPu",           ЭлементКоллекции.КоличествоУпаковок);
...
//		ПараметрыЗапроса.Вставить("ЧастичнаяПриемка", ЧастичнаяПриемка);
//		Если ПараметрыЗапроса.ЧастичнаяПриемка Тогда
//			СообщениеJSON.ПараметрыЗапроса.Вставить("Операция", Перечисления.ВидыОперацийСАТУРН.НакладнаяПодтверждениеЧастичногоВозврата);
//		Иначе
//			СообщениеJSON.ПараметрыЗапроса.Вставить("Операция", Перечисления.ВидыОперацийСАТУРН.НакладнаяПодтверждениеВозврата);
//		КонецЕсли;
//	Иначе
//		СообщениеJSON.ПараметрыЗапроса.Вставить("Операция", Перечисления.ВидыОперацийСАТУРН.НакладнаяПодтверждениеУтериГрузаПриВозврате);
//	КонецЕсли;
