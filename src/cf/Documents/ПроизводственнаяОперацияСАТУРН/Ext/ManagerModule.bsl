...
Функция ПроизводственнаяОперацияJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	|
	|	ПРЕДСТАВЛЕНИЕ(Сырье.Партия) КАК ПредставлениеПартии,
	|	ЕСТЬNULL(Сырье.Партия.УпаковочнаяЕдиница.КоличествоСАТУРН, 0) КАК КоличествоВУпаковкеПартии,
	...
	ОшибкиУпаковок = ИнтеграцияСАТУРНСлужебный.ОшибкиКоличествоВУпаковкеНеСоответствуетКоличествуВУпаковкеПартии(Сырье);
	Если ОшибкиУпаковок.Количество() > 0 Тогда
		СообщениеJSON.ТекстОшибки = СтрСоединить(ОшибкиУпаковок, Символы.ПС);
		СообщенияJSON.Добавить(СообщениеJSON);
		Возврат СообщенияJSON;
	КонецЕсли;
	...
	ИнтерфейсСАТУРН.ДобавитьАргументШапки(АргументыОперации, "docDate",              Шапка.ДатаПроизводства);
