...
Процедура СформироватьСтрокуРасчетноПлатежныхДокументов(НовыйАдресВХранилище)
	...
	Для Каждого СтрокаТаблицы Из Объект.ПлатежноРасчетныеДокументы Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.НомерСчетаФактурыНаАванс) Тогда
			...
			СтрокаНомеровИДатаАвансов = СтрокаНомеровИДатаАвансов 
				+ ?(Не ЗначениеЗаполнено(СтрокаНомеровИДатаАвансов), "", "; ") + АвансовыеСчетаФактурыНомерДата;
		КонецЕсли;
