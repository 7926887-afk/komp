...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	НЕ &НесколькоВалют
	|	И НачисленияКредитовИДепозитов.ВалютаВзаиморасчетов = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)";
	...
	Пока Документ.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				#Область ЗаполнитьВалютуВзаиморасчетов
				Рекомендация = НСтр("ru = 'Перезаполните таблицу ""Начисления"" вручную.'");
				Для Каждого Запись Из ДокументОбъект.Начисления Цикл
					Если НЕ ЗначениеЗаполнено(Запись.ВалютаВзаиморасчетов) Тогда
