...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = 
		...
		|	ТИПЗНАЧЕНИЯ(ОперацииДокумента.АналитикаАктивовПассивов) = ТИП(Перечисление.УдалитьТипыНалогов) КАК ЗаполнятьРеквизитТипНалога,
		|	ОперацииДокумента.УдалитьСуммаНеОблагаемаяНДС > 0 КАК ПереноситьНеоблагаемуюСуммуВОперации,
		|	МАКСИМУМ(ВЫБОР
		...
		|	ТИПЗНАЧЕНИЯ(ОперацииДокумента.АналитикаАктивовПассивов) = ТИП(Перечисление.УдалитьТипыНалогов),
		|	ОперацииДокумента.УдалитьСуммаНеОблагаемаяНДС > 0
		|";
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка 
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ВыборкаДокументов.ЗаполнятьРеквизитТипНалога Тогда
					Перечисления.УдалитьТипыНалогов.ЗаполнитьРеквизитТипНалога(ДокументОбъект, "Операции.АналитикаАктивовПассивов");
				КонецЕсли;
				
				Если ВыборкаДокументов.ПереноситьНеоблагаемуюСуммуВОперации Тогда
					...
					ПересчитатьИтоговуюСуммуОпераций(ДокументОбъект);
...
Процедура ПересчитатьИтоговуюСуммуОпераций(ДокументОбъект)
