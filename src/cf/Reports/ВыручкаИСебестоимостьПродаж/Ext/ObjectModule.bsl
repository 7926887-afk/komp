...
Функция ТекстЗапроса(ПоказыватьПродажи)
	...
	|ИЗ   
	|	РегистрНакопления.ВыручкаИСебестоимостьПродаж.Обороты(ДАТАВРЕМЯ(1,1,1) {(ДАТАВРЕМЯ(1,1,1))}, &КонецПериода {(&КонецПериода)}, Авто,
	|		(АналитикаУчетаНоменклатуры, ЗаказКлиента) В (ВЫБРАТЬ АналитикаУчетаНоменклатуры, ЗаказКлиента ИЗ АналитикиДляОтбора КАК АналитикиДляОтбора)
	|		И &ТекстУсловияОтбораАналитикаУчетаПоПартнерам
	|	) КАК ИсходныеПродажи
	|ГДЕ
