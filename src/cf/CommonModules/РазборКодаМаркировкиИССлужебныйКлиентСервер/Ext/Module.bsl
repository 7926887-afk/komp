...
Функция ОбработатьРезультатНесколькихШаблонов(Результат, ДанныеРезультата)
	...
	Если Результат.Количество() = 2 Тогда
		...
		Если Результат[0].ВидыПродукции.Найти(ВидПродукцииТабак) <> Неопределено
		...
		// Если есть шаблон кода маркировки и шаблон АТК, то используем шаблон кода маркировки
		Если Результат[0].ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.АгрегированныйТаможенныйКод")
			И ЗначениеЗаполнено(Результат[1].ТипШтрихкода) Тогда
				ДанныеРезультата = Результат[1];
				Возврат Истина;
		ИначеЕсли Результат[1].ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.АгрегированныйТаможенныйКод")
			И ЗначениеЗаполнено(Результат[0].ТипШтрихкода) Тогда
				ДанныеРезультата = Результат[0];
				Возврат Истина;
		КонецЕсли;
...
// * РезультатРазбора - см. РазборКодаМаркировкиИССлужебныйКлиентСервер.РазобратьСтрокуШтрихкодаGS1, Неопределено - Предварительный разбор при флаге СодержитРазделительGS.
...
Функция ИнициализироватьНастройкиШаблонаРазобранногоКодаМаркировки()
	...
	// Детализированные по GTIN
	ПараметрыШаблона.Вставить("GTIN", Новый Соответствие);
...
Функция РазобратьКодМаркировкиПолностью(ПараметрыОбработкиКодаМаркировки, ПримечаниеКРезультатуРазбора, КешНастроекРазбораКодаМаркировки)
	...
	GTIN = Неопределено;
	РезультатРазбора.СоставКодаМаркировки.Свойство("GTIN", GTIN);
	...
	Иначе
		Для Каждого ПараметрыШаблона Из ДопустимыеШаблоны Цикл
			Если ПараметрыШаблона.ШаблонРазбораКодаМаркировки = ДанныеРасширеннойДетализации.ШаблонРазбораКодаМаркировки Тогда
				ДобавитьШаблон = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	ПараметрыШаблона.GTIN.Вставить(GTIN, ПараметрыGTIN(РезультатРазбора));
	
	Возврат РезультатРазбора;
	
КонецФункции

Функция ПараметрыGTIN(РезультатРазбора)
	
	Результат = Новый Структура;
	Результат.Вставить("ВидУпаковки",                  РезультатРазбора.ВидУпаковки);
	Результат.Вставить("ВидыУпаковокПоВидамПродукции", РезультатРазбора.ВидыУпаковокПоВидамПродукции);
	Результат.Вставить("ВидыПродукции",                РезультатРазбора.ВидыПродукции);
	Возврат Результат;
...
Функция РазобратьКодМаркировкиУпрощенноПоШаблону(ПараметрыОбработкиКодаМаркировки, ПараметрыПроверяемогоШаблона, КешНастроекРазбораКодаМаркировки)
	...
	Если ЗначениеИдентификатораGTIN <> Неопределено Тогда
		...
		Если ПараметрыПроверяемогоШаблона.GTIN.Получить(ЗначениеИдентификатораGTIN) = Неопределено Тогда
	...
	Для Каждого ОписаниеЭлементаКМ Из ДанныеШаблона.ОписаниеЭлементовКодаМаркировки Цикл
		...
		Если ОписаниеЭлементаКМ.Имя = "МРЦСтрокой" Или ОписаниеЭлементаКМ.Имя = "ШтрихкодАкцизнойМарки" Тогда
			...
			Если ОписаниеЭлементаКМ.Имя = "МРЦСтрокой" Тогда
				ПараметрыОписанияКодаМаркировки.Вставить("ВидУпаковки", ПараметрыПроверяемогоШаблона.GTIN[ЗначениеИдентификатораGTIN].ВидУпаковки);
	...
	ДанныеРезультата.ВидУпаковки                  = ПараметрыПроверяемогоШаблона.GTIN[ЗначениеИдентификатораGTIN].ВидУпаковки;
	ДанныеРезультата.ВидыПродукции                = ПараметрыПроверяемогоШаблона.GTIN[ЗначениеИдентификатораGTIN].ВидыПродукции;
	ДанныеРезультата.ВидыУпаковокПоВидамПродукции = ПараметрыПроверяемогоШаблона.GTIN[ЗначениеИдентификатораGTIN].ВидыУпаковокПоВидамПродукции;
