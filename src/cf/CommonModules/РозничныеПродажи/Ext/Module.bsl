...
Процедура УдалениеЧековККМ() Экспорт
	...
	Запрос = Новый Запрос;
 	...
 	// Выбираются кассовые смены:
	//		новая с чеками
	// 		новая без чеков
	//		архивная с любыми чеками
	//
	// Архивные без чеков исключаются
	
	Запрос.Текст = "ВЫБРАТЬ
		   |	ЧекККМ.КассоваяСмена КАК КассоваяСмена,
		   |	КассовыеСмены.Дата КАК ДатаКассовойСмены
		   |ИЗ
		   |	Документ.ЧекККМ КАК ЧекККМ
		   |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КассоваяСмена КАК КассовыеСмены
		   |		ПО (КассовыеСмены.Ссылка = ЧекККМ.КассоваяСмена)
		   |ГДЕ
		   |	ЧекККМ.КассоваяСмена.Проведен
		   |	И ЧекККМ.КассоваяСмена.Дата < &ДатаХраненияЧеков
		   |	И ЧекККМ.КассоваяСмена.СтатусРегламентныхОпераций = ЗНАЧЕНИЕ(Перечисление.СтатусыРегламентныхОперацийКассовойСмены.СозданОтчетЧекиЗаархивированы)
		   |	И (ЧекККМ.Проведен
		   ...
		   |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КассоваяСмена КАК КассовыеСмены
		   |		ПО (КассовыеСмены.Ссылка = ЧекККМВозврат.КассоваяСмена)
		   |ГДЕ
		   |	ЧекККМВозврат.КассоваяСмена.Проведен
		   |	И ЧекККМВозврат.КассоваяСмена.Дата < &ДатаХраненияЧеков
		   |	И ЧекККМВозврат.КассоваяСмена.СтатусРегламентныхОпераций = ЗНАЧЕНИЕ(Перечисление.СтатусыРегламентныхОперацийКассовойСмены.СозданОтчетЧекиЗаархивированы)
		   ...
		   |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КассоваяСмена КАК КассовыеСмены
		   |		ПО (КассовыеСмены.Ссылка = ЧекККМКоррекции.КассоваяСмена)
		   |ГДЕ
		   |	ЧекККМКоррекции.КассоваяСмена.Проведен
		   |	И ЧекККМКоррекции.КассоваяСмена.Дата < &ДатаХраненияЧеков
		   |	И ЧекККМКоррекции.КассоваяСмена.СтатусРегламентныхОпераций = ЗНАЧЕНИЕ(Перечисление.СтатусыРегламентныхОперацийКассовойСмены.СозданОтчетЧекиЗаархивированы)
		   |	И (ЧекККМКоррекции.Проведен
		   |			ИЛИ &ВключатьНепроведенныеЧеки
		   |				И НЕ ЧекККМКоррекции.ПометкаУдаления)
		   ...
		   |ГДЕ
		   |	КассовыеСмены.Проведен
		   |	И КассовыеСмены.Дата < &ДатаХраненияЧеков
		   |	И КассовыеСмены.СтатусРегламентныхОпераций = ЗНАЧЕНИЕ(Перечисление.СтатусыРегламентныхОперацийКассовойСмены.СозданОтчет)
		   |
		   |УПОРЯДОЧИТЬ ПО
		   |	ДатаКассовойСмены,
		   |	КассоваяСмена";
	...
	ВключатьНепроведенныеЧеки = Ложь;
	Запрос.УстановитьПараметр("ВключатьНепроведенныеЧеки", ВключатьНепроведенныеЧеки);
	Результат = Запрос.Выполнить();
	
	Если НЕ Результат.Пустой() Тогда
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление чеков ККМ'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация, , ,
...
Процедура ВыполнитьАрхивациюЧековККМ(ПараметрыЗадания, АдресХранилища="", ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	// Обрабатываются кассовые смены:
	//		новая с любыми чеками
	//		новая без чеков
	//		архивная с не архивными чеками

	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ЧекККМ.Ссылка КАК Ссылка,
	|	ЧекККМ.КассоваяСмена КАК КассоваяСмена
	|ПОМЕСТИТЬ ЧекиККМДляАрхивации
	|ИЗ
	|	Документ.ЧекККМ КАК ЧекККМ
	|ГДЕ
	|	НЕ ЧекККМ.Архивный
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЧекиККМДляАрхивации.КассоваяСмена КАК Ссылка,
	|	ДокументКассоваяСмена.Дата КАК Дата
	|ПОМЕСТИТЬ КассовыеСменыДляОбработки
	|ИЗ
	|	ЧекиККМДляАрхивации КАК ЧекиККМДляАрхивации
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КассоваяСмена КАК ДокументКассоваяСмена
	|		ПО (ДокументКассоваяСмена.Ссылка = ЧекиККМДляАрхивации.КассоваяСмена)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|	И КассоваяСмена.СтатусРегламентныхОпераций = ЗНАЧЕНИЕ(Перечисление.СтатусыРегламентныхОперацийКассовойСмены.СозданОтчет)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЧекиККМДляАрхивации.КассоваяСмена
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЧекиККМДляАрхивации.Ссылка КАК Ссылка,
	|	ЧекиККМДляАрхивации.КассоваяСмена КАК КассоваяСмена,
	|	ДокументКассоваяСмена.Дата КАК ДатаКассовойСмены
	|ИЗ
	|	ЧекиККМДляАрхивации КАК ЧекиККМДляАрхивации
	|		ЛЕВОЕ СОЕДИНЕНИЕ КассовыеСменыДляОбработки КАК ДокументКассоваяСмена
	|		ПО (ДокументКассоваяСмена.Ссылка = ЧекиККМДляАрхивации.КассоваяСмена)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ОтчетОРозничныхВозвратах.Ссылка, ЗНАЧЕНИЕ(Документ.ОтчетОРозничныхВозвратах.ПустаяСсылка)),
	|	ДокументКассоваяСмена.Ссылка,
	|	ДокументКассоваяСмена.Дата
	|ИЗ
	|	КассовыеСменыДляОбработки КАК ДокументКассоваяСмена
	...
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ОтчетОРозничныхПродажах.Ссылка, ЗНАЧЕНИЕ(Документ.ОтчетОРозничныхПродажах.ПустаяСсылка)),
	|	ДокументКассоваяСмена.Ссылка,
	|	ДокументКассоваяСмена.Дата
	|ИЗ
	|	КассовыеСменыДляОбработки КАК ДокументКассоваяСмена
...
Функция ЗаполнитьОтчетОРозничныхПродажах(ОтчетОРозничныхПродажахОбъект, СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, ТолькоЗаполнить = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ ЧекиККМСмешаннойОплатыПоКоторымОформленПолныйВозвратВСмену КАК ЧекиККМСмешаннойОплатыПолностьюВозвращенные
		|			ПО ЧекиККМСмешаннойОплатыПолностьюВозвращенные.ЧекККМ = ЧекККМОплатаПлатежнымиКартами.Ссылка 
		|	ГДЕ
		|		ЧекиККМ.ПоЧекуКоррекции = ЛОЖЬ
		|		И ЧекиККМСмешаннойОплатыПолностьюВозвращенные.ЧекККМ ЕСТЬ NULL
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ ЧекиККМСмешаннойОплатыПоКоторымОформленПолныйВозвратВСмену КАК ЧекиККМСмешаннойОплатыПолностьюВозвращенные
		|			ПО ЧекиККМСмешаннойОплатыПолностьюВозвращенные.ЧекККМ = ЧекиККМ.ЧекККМ 
		|	ГДЕ
		...
		|		ЧекиККМ.ВидЦены КАК ВидЦены
		|	ИЗ
		|		ЧекиККМСмешаннойОплатыПоКоторымОформленПолныйВозвратВСмену КАК ЧекиККМСмешаннойОплатыПоКоторымОформленПолныйВозвратВСмену		
		|			ЛЕВОЕ СОЕДИНЕНИЕ ЧекиККМ КАК ЧекиККМ
		|			ПО ЧекиККМСмешаннойОплатыПоКоторымОформленПолныйВозвратВСмену.ЧекККМ = ЧекиККМ.Ссылка
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМ.ОплатаПлатежнымиКартами КАК ЧекККМОплатаПлатежнымиКартами
		|			ПО ЧекККМОплатаПлатежнымиКартами.Ссылка = ЧекиККМ.Ссылка
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		// Возвраты по полностью возвращенным чекам в смену, которые были оплачены смешанно
		|	ВЫБРАТЬ
		...
		|	ИЗ
		|		ЧекиККМ КАК ЧекиККМ
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЧекиККМСмешаннойОплатыПоКоторымОформленПолныйВозвратВСмену КАК ЧекиККМСмешаннойОплатыПоКоторымОформленПолныйВозвратВСмену
		|			ПО ЧекиККМСмешаннойОплатыПоКоторымОформленПолныйВозвратВСмену.ЧекККМ = ЧекиККМ.ЧекККМ
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЧекККМВозврат.ОплатаПлатежнымиКартами КАК ЧекККМВозвратОплатаПлатежнымиКартами
		|			ПО ЧекККМВозвратОплатаПлатежнымиКартами.Ссылка = ЧекиККМ.Ссылка
		|	ГДЕ
...
Функция ПодготовитьДанныеПоЧекамКассовойСмены(СостояниеКассовойСмены, МенеджерВременныхТаблиц, ВключатьНепроведенныеЧеки = Ложь) Экспорт
	...
	Запрос.Текст = "
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПолныеВозвратыВСмену.ЧекККМ КАК ЧекККМ
		|ПОМЕСТИТЬ ЧекиККМСмешаннойОплатыПоКоторымОформленПолныйВозвратВСмену
		|ИЗ
		|	(ВЫБРАТЬ 
		|		ЧекиККМПоКоторымОформленВозвратВСмену.ЧекККМ КАК ЧекККМ,
		|		ЧекиККМПоКоторымОформленВозвратВСмену.СуммаДокумента КАК СуммаДокумента
		|	ИЗ ЧекиККМПоКоторымОформленВозвратВСмену КАК ЧекиККМПоКоторымОформленВозвратВСмену
		|	ГДЕ ЧекиККМПоКоторымОформленВозвратВСмену.ПолученоНаличными <> ЧекиККМПоКоторымОформленВозвратВСмену.СуммаДокумента 
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ 
		|		ЧекиККМВозврат.ЧекККМ КАК ЧекККМ,
		|		-ЧекиККМВозврат.СуммаДокумента КАК СуммаДокумента
		|	ИЗ	
		|		Документ.ЧекККМВозврат КАК ЧекиККМВозврат
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЧекиККМПоКоторымОформленВозвратВСмену КАК ЧекиККМПоКоторымОформленВозвратВСмену
		|			ПО ЧекиККМВозврат.ЧекККМ = ЧекиККМПоКоторымОформленВозвратВСмену.ЧекККМ
		|	ГДЕ
		|		ЧекиККМВозврат.КассоваяСмена = &КассоваяСмена
		|		И (ЧекиККМВозврат.Проведен
		|			ИЛИ &ВключатьНепроведенныеЧеки И НЕ ЧекиККМВозврат.ПометкаУдаления)
		|		И ЧекиККМВозврат.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековККМ.Пробит)
		|		И ЧекиККМПоКоторымОформленВозвратВСмену.ПолученоНаличными <> ЧекиККМПоКоторымОформленВозвратВСмену.СуммаДокумента 
		|	) КАК ПолныеВозвратыВСмену
		|
		|СГРУППИРОВАТЬ ПО 
		|	ПолныеВозвратыВСмену.ЧекККМ
		|
		|ИМЕЮЩИЕ
		|	СУММА(ПолныеВозвратыВСмену.СуммаДокумента) = 0
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЧекККМ
		|;
