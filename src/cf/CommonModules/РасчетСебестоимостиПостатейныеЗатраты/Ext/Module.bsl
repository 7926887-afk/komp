...
Функция ТекстПриобретенияПоТаможеннымДекларациям()
	...
	ТекстЗапроса =
		...
		|					ИСТИНА
		|				ИЗ
		|					ВТДвиженияСебестоимостьТоваровПоДекларациям КАК Себестоимость
		|				ГДЕ
		|					Себестоимость.Регистратор = Декларации.Ссылка)
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТДвиженияСебестоимостьТоваровПоДекларациям
		|;
		|УНИЧТОЖИТЬ ДекларацииБезНДСНулеваяТаможеннаяПошлина
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция ТекстРассчитаннаяСтоимостьПартийТоваров()
	...
	// Условия выборки должны соответствать запросам выборки базы распределения:
	// - ШаблонТекстаПервичныеПриемникиПоСправочникам()
	// - ШаблонТекстаПервичныеПриемникиПоПереработке()
	|	И (Себестоимость.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия)
	|		ИЛИ Себестоимость.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Перемещение)
	|		И Себестоимость.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеПереработчику)
	|		И Себестоимость.АналитикаУчетаНоменклатуры.ТипМестаХранения = ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.ДоговорКонтрагента)
	|		)
