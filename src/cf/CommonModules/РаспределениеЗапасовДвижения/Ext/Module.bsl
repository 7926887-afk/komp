...
Процедура ЗаписатьРаспределениеЗапасовНаПотребностиПоТаблицеРазличныеТовары(Запрос) Экспорт
	...
	Запрос.Текст =
		...
		|	Остатки.РезервироватьНаСкладеПоВсемЗаказамСводно   КАК РезервироватьНаСкладеПоВсемЗаказамСводно,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не Выборка.РаспределятьЕслиОстатокНеотрицательный Или Выборка.Свободно >= 0 Тогда
			...
			КоличествоРаспределить = Мин(Количество,
				Макс(Выборка.Запас - Выборка.РезервироватьНаСкладеПоВсемЗаказамСводно, 0));
			Свободно = Выборка.Запас - Выборка.РезервироватьНаСкладеПоВсемЗаказамСводно - КоличествоРаспределить;
			...
			Если Выборка.Запас <> 0
					Или Выборка.РезервироватьНаСкладеПоВсемЗаказамСводно <> 0
					Или Свободно <> 0
					Или Излишек <> 0
