...
Процедура ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры, ДополнительныеСвойстваПроведения = Неопределено) Экспорт
	...
	Для Каждого СтрокаОсновныхПараметровРасчета Из ТаблицаОсновныхПараметровРасчета Цикл
		...
		ТаблицаЗачетовОплат.Колонки.Добавить("ПорядокЗачета", ОбщегоНазначения.ОписаниеТипаСтрока(40));
			...
			Если Индексы.ИндексОплаты <= ЗаписейОплаты - 1 И Индексы.ИндексОтгрузкиПоставки <= ЗаписейОтгрузкиПоставки - 1 Тогда 
				...
				Если СтрокаОтгрузкиПоставки.СуммаДляРаспределения > 0 И СтрокаОплаты.СуммаДляРаспределения > 0 Тогда
					...
					Если СтрокаОплаты.ПорядокОперации < СтрокаОтгрузкиПоставки.ПорядокОперации 
						...
						Если ПропускОплаты = 0 И СтрокаОсновныхПараметровРасчета.ЗачетОплатПоДатеПлатежа Тогда
					...
					Иначе
						...
						Если НЕ СтрокаОплаты.ЭтоКорректировка 
							...
							Если СтрокаОтгрузкиПоставки.СуммаДляРаспределения > 0 И СтрокаОплаты.СуммаДляРаспределения > 0 Тогда
								...
								// Поищем их и увеличим пропуск строк отгрузки поставки
								Если ПропускОтгрузкиПоставки = 0 И СтрокаОсновныхПараметровРасчета.ЗачетОплатПоДатеПлатежа Тогда
		...
		Для Каждого СтрокаЗачета Из ТаблицаЗачетовОплат Цикл
			...
			Если СтрокаЗачета.ЗачетАванса Тогда
				...
				Если (СтрокаОтгрузкиПоставки.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВзаимозачетЗадолженности
						ИЛИ СтрокаОтгрузкиПоставки.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИспользованиеБронированияПодотчетнымЛицом)
				...
				Если СтрокаЗачета.ПорядокЗачета >= ГлобальныеПеременные.ПорядокФакт Тогда
					...
					НовСтр_Предоплата_Расход.ТипЗаписиВзаиморасчетов = ТипЗаписиВзаиморасчетов(СтрокаОтгрузкиПоставки.ОперацияВзаиморасчетов, НовСтр_Предоплата_Расход);
				КонецЕсли;
				...
				Если НЕ ДвойнаяЗаписьНеНужна Тогда
					...
					#Область ПриходДолг
						...
						Если НовСтр_Долг_Приход.Сторно
							И СтрокаОтгрузкиПоставки.ОперацияВзаиморасчетов = Перечисления.ОперацииВзаиморасчетов.Оплата Тогда
							НовСтр_Долг_Приход.ТипЗаписиВзаиморасчетов = ТипЗаписиВзаиморасчетов(Перечисления.ОперацииВзаиморасчетов.ПереносВзаиморасчетов, НовСтр_Долг_Приход);
						Иначе
							НовСтр_Долг_Приход.ТипЗаписиВзаиморасчетов = ТипЗаписиВзаиморасчетов(СтрокаОтгрузкиПоставки.ОперацияВзаиморасчетов, НовСтр_Долг_Приход);
						КонецЕсли;
					...
					Если НовСтр_Долг_Расход.Сторно
						И СтрокаОтгрузкиПоставки.ОперацияВзаиморасчетов = Перечисления.ОперацииВзаиморасчетов.Оплата Тогда
						НовСтр_Долг_Расход.ТипЗаписиВзаиморасчетов = ТипЗаписиВзаиморасчетов(Перечисления.ОперацииВзаиморасчетов.ПереносВзаиморасчетов, НовСтр_Долг_Расход);
					Иначе
						НовСтр_Долг_Расход.ТипЗаписиВзаиморасчетов = ТипЗаписиВзаиморасчетов(СтрокаОтгрузкиПоставки.ОперацияВзаиморасчетов, НовСтр_Долг_Расход);
					КонецЕсли;
					КонецЕсли;
					#КонецОбласти
			...
			Иначе
				...
				Если НЕ СтрокаЗачета.ЭтоКорректировка Тогда
					...
					Если СтрокаЗачета.ПорядокЗачета >= ГлобальныеПеременные.ПорядокФакт Тогда
						...
						НовСтр_Долг_Расход.ТипЗаписиВзаиморасчетов = ТипЗаписиВзаиморасчетов(СтрокаОплаты.ОперацияВзаиморасчетов, НовСтр_Долг_Расход);
					КонецЕсли;
					#КонецОбласти
				...
				Иначе
					...
					Если СтрокаЗачета.ПорядокЗачета >= ГлобальныеПеременные.ПорядокФакт Тогда
						...
						НовСтр_Сторно.ТипЗаписиВзаиморасчетов = ТипЗаписиВзаиморасчетов(СтрокаОплаты.ОперацияВзаиморасчетов, НовСтр_Сторно);
		...
		ТаблицаРасчетовПоСрокам.Свернуть("Период, Регистратор, ВидДвижения, АналитикаУчетаПоПартнерам, ОбъектРасчетов, Валюта, ДокументРегистратор,
				...
				|НастройкаХозяйственнойОперации,ИдентификаторФинЗаписи,ЗаписьДоНачалаРасчета,ПоДаннымОбъектаРасчетовИсточника,ТипЗаписиВзаиморасчетов",
				"Предоплата, ПредоплатаРегл, ПредоплатаУпр, Долг, ДолгРегл, ДолгУпр,СуммаПриемник");
		
		Если Не ОбщегоНазначенияУТПовтИсп.ЗначениеКонстанты("УдалитьТипыЗаписейВзаиморасчетовЗаполнены") Тогда
...
Функция ТекстЗапросаРасчетыПланОплатОстатки(ГлобальныеПеременные, РасчетыСКлиентами = Истина, ИмяТаблицы = "")
	...
	|	ОстаткиИОбороты.ДатаПлановогоПогашения КАК ДатаПлановогоПогашения,
	|	ОстаткиИОбороты.ДатаВозникновения КАК ДатаВозникновения,
	|	ВтОсновныеПараметрыРасчета.ПараметрПорядок КАК ПорядокОперации,
	...
	|ИЗ
	|	&ТекстПодзапроса КАК ОстаткиИОбороты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтОсновныеПараметрыРасчета КАК ВтОсновныеПараметрыРасчета
	|			ПО ВтОсновныеПараметрыРасчета.ПараметрАналитикаУчетаПоПартнерам = ОстаткиИОбороты.АналитикаУчетаПоПартнерам
	|				И ВтОсновныеПараметрыРасчета.ПараметрОбъектРасчетов = ОстаткиИОбороты.ОбъектРасчетов
	|				И ВтОсновныеПараметрыРасчета.ПараметрВалютаРасчетов = ОстаткиИОбороты.Валюта
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиИОбороты.АналитикаУчетаПоПартнерам,
	|	ОстаткиИОбороты.ОбъектРасчетов,
	|	ОстаткиИОбороты.Валюта,
	...
	|	ВтОсновныеПараметрыРасчета.ПараметрПорядок
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОбъектРасчетов,
	|	АналитикаУчетаПоПартнерам,
...
Функция ПолучитьОплаты(Запрос, ГлобальныеПеременные, ЭтоРасчетыСКлиентами = Ложь)
	...
	Запрос.Текст = "
		...
		|		
		|		ВЫБОР 
		|			КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.БронированиеЧерезАгента)
		|				ИЛИ Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.БронированиеУПоставщика) 
		...
		|	ТаблицаОплат.ОперацияВзаиморасчетов                      КАК ОперацияВзаиморасчетов,
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные, ЭтоРасчетыСКлиентами = Ложь)
	...
	|		ИНАЧЕ Расчеты.ДатаПлатежа
	|	КОНЕЦ , ДЕНЬ),
	|	Расчеты.ОперацияВзаиморасчетов,
	...
	|	РасчетыОтгрузкиПоставки.ОперацияВзаиморасчетов         КАК ОперацияВзаиморасчетов,
	...
	|	РасчетыПоСрокам.РасчетныйДокумент                 КАК РасчетныйДокумент,
	|	РасчетыПоСрокам.ОперацияВзаиморасчетов            КАК ОперацияВзаиморасчетов,
	...
	|	РасчетыПоСрокам.РасчетныйДокумент,
	|	РасчетыПоСрокам.ОперацияВзаиморасчетов,
...
Процедура ДобавитьПриход(ГлобальныеПеременные, ТаблицаДвиженийРасчетов, СтрокаРасчетов, ТипИсточника, ТаблицаЗачетовОплат, Индекс)
	...
	Для Каждого СтрокаЗачета Из СтрокиЗачетов Цикл
		...
		Если СтрокаРасчетов.ПорядокОперации >= ГлобальныеПеременные.ПорядокФакт Тогда
			...
			КонецЕсли;
			НовСтр.ТипЗаписиВзаиморасчетов = ТипЗаписиВзаиморасчетов(СтрокаРасчетов.ОперацияВзаиморасчетов, НовСтр);
...
Процедура ДобавитьЗаписьПереоценки(ГлобальныеПеременные, ТаблицаРасчетовПоДокументам, Период, СтрокаРасчетов, Тип, СуммаРегл, СуммаУпр)
	...
	Если СуммаРегл < 0 ИЛИ СуммаУпр < 0 Тогда
		...
		КонецЕсли;
		СтрокаПереоценки.ТипЗаписиВзаиморасчетов = ТипЗаписиВзаиморасчетов(Перечисления.ОперацииВзаиморасчетов.КорректировкаЗадолженности, СтрокаПереоценки);
...
Функция ТаблицаИзмененийДляПересчета(МенеджерВременныхТаблиц, Регистратор) Экспорт
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти("РасчетыСКлиентамиИзменения") <> Неопределено Тогда
		...
		|				ИЛИ ТИПЗНАЧЕНИЯ(&Регистратор) = ТИП(Документ.ВводОстатковВзаиморасчетов)
		|				ТОГДА """"
...
Функция ТипЗаписиВзаиморасчетов(ОперацияВзаиморасчетов, ЗаписьТаблицы)
	
	Если Не ОбщегоНазначенияУТПовтИсп.ЗначениеКонстанты("УдалитьТипыЗаписейВзаиморасчетовЗаполнены") Тогда
		Возврат Перечисления.ТипыЗаписейВзаиморасчетов.ПустаяСсылка();
	КонецЕсли;
	
	Сторно = ЗаписьТаблицы.Сторно;
	Если ТипЗнч(ЗаписьТаблицы.Сторно) <> Тип("Булево") Тогда
		Сторно = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьТипЗаписиВзаиморасчетов(Запись, ТипРегистратора) Экспорт
	...
	Если Не ЗначениеЗаполнено(Запись.Сторно) Тогда
		Запись.Сторно = Ложь;
	КонецЕсли;
