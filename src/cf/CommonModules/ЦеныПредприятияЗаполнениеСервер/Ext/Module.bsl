...
Функция ПолучитьТекстЗапросаВременнойТаблицыЦенПоСоглашению2_5(ИмяВременнойТаблицыТоваров)
	...
	|							Т.Номенклатура
	|						ИЗ
	|							ТаблицаОтбора КАК Т)) КАК СтавкиНДСНоменклатуры
	|		ПО (ВременнаяТаблицаТовары.Номенклатура = СтавкиНДСНоменклатуры.Номенклатура)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОсновныеСтавкиНДС.СрезПоследних(&Дата, Страна = &СтранаРегистрации) КАК ОсновныеСтавкиНДС
	|		ПО (ИСТИНА)
	|
	...
	|ВЫБРАТЬ
	|	втТаблицаТовары.НомерСтроки КАК НомерСтроки,
	|	ЕСТЬNULL(втТаблицаЦеныПоСоглашению.ВидЦены, ЗНАЧЕНИЕ(Справочник.ВидыЦен.ПустаяСсылка)) КАК ВидЦены,
	|	ЕСТЬNULL(втТаблицаЦеныПоСоглашению.СтавкаНДС, ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)) КАК СтавкаНДС,
	|	ЕСТЬNULL(втТаблицаЦеныПоСоглашению.Цена, 0) КАК Цена
	|ИЗ
	|	&ИмяВременнойТаблицыТоваров КАК втТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ втТаблицаЦеныПоСоглашению КАК втТаблицаЦеныПоСоглашению
	|		ПО (втТаблицаЦеныПоСоглашению.НомерСтроки = втТаблицаТовары.НомерСтроки)
	|			И ((втТаблицаЦеныПоСоглашению.Номенклатура, втТаблицаЦеныПоСоглашению.ХарактеристикаЦО, втТаблицаЦеныПоСоглашению.СерияЦО, втТаблицаЦеныПоСоглашению.УпаковкаЦО, втТаблицаЦеныПоСоглашению.Приоритет) В
	|				(ВЫБРАТЬ
	...
	|					втТаблицаЦеныПоСоглашению.УпаковкаЦО))
	|";
