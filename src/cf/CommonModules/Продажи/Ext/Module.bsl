	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияРаспоряженияНаОтгрузкуИзменение") Тогда
		...
		Если ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров = 0 Тогда
			...
			ТекстЗапроса =
				...
				|	СУММА(ТаблицаОстатков.КОформлению)             КАК КОформлению,
				|	СУММА(ТаблицаОстатков.КПередаче)               КАК КПередаче
				|ИЗ
				|	ДвиженияРаспоряженияНаОтгрузкуИзменение КАК Таблица
				|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РаспоряженияНаОтгрузку КАК ТаблицаОстатков
				|		ПО  ТаблицаОстатков.Распоряжение = Таблица.Распоряжение
				|			И ТаблицаОстатков.Номенклатура = Таблица.Номенклатура
				|			И ТаблицаОстатков.Характеристика = Таблица.Характеристика
				|			И ТаблицаОстатков.Серия = Таблица.Серия
				...
				|
				|ИМЕЮЩИЕ
				|	СУММА(ТаблицаОстатков.КОформлению) < 0
				|	ИЛИ СУММА(ТаблицаОстатков.КПередаче) < 0
				|";
