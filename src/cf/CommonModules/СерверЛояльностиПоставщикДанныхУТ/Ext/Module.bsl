...
Процедура ЗаполнитьТаблицуУправляемыхСкидок(Результат, Знач ПараметрыРасчета) Экспорт
	...
	НастрокаОтборов = ЗаполнитьТаблицуУправляемыхСкидокПараметрыРасчета(ПараметрыРасчета);
	ДобавляемыеОтборы = Новый Массив;
	ПараметрыОтбора = Новый Структура;
	Если НастрокаОтборов.ИспользоватьОтборы Тогда
		КлючОтбораИмя = "Магазин";
		Если ЗначениеЗаполнено(НастрокаОтборов[КлючОтбораИмя]) Тогда
			ДобавляемыеОтборы.Добавить("СкидкаНаценка.ВидЦены В (Выбрать ВидЦены из ОтборПоВидуЦен)");
		КонецЕсли;
		ПараметрыОтбора.Вставить(КлючОтбораИмя, НастрокаОтборов[КлючОтбораИмя]);
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.ДействиеСкидокНаценок.СрезПоследних(&СкидкиНаДату, &УсловиеВТ) КАК ДействиеСкидокНаценокСрезПоследних
		|ГДЕ
		|	ДействиеСкидокНаценокСрезПоследних.СкидкаНаценка.Управляемая
		|	И ДействиеСкидокНаценокСрезПоследних.Статус = &СтатусДействует
		|";
	
	Для каждого Элем Из ПараметрыОтбора Цикл
		Запрос.Параметры.Вставить(Элем.Ключ, Элем.Значение);
	КонецЦикла;
	ОтборыСтрокой = "";
	Если ДобавляемыеОтборы.Количество() Тогда
		ОтборыСтрокой = СтрСоединить(ДобавляемыеОтборы, " И ");
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеВТ", ОтборыСтрокой);
