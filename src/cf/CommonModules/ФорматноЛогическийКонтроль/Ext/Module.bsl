...
Функция ВыполненаПроверкаОбязательностиИПравильностиЗаполненияТэгов(ДанныеЧека, ИдентификаторУстройства, Ошибки = Неопределено, ИсправленыОсновныеПараметры = Неопределено) Экспорт
	...
	ПроверитьСведенияОРасчетеВИнтернет(Проверка, ДанныеЧека, СуммыОплат);     
...
Процедура ПроверитьКодыНалогообложения(Проверка, Данные, ДоступныеКодыСистемыНалогообложения)
	...
	Если Проверка.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККТ Тогда
		...
		Если Данные.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.ЕНВД Тогда
			ТекстОшибки = НСтр("ru = 'Формирование чека по СНО ЕНВД устарело, не предусмотрено ФФД.'");
...
// Выполняет проверку сведений О расчете в интернет
//
// Параметры:
//   Проверка - см. ПараметрыПроверки.
//   Данные - см. ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыОперацииФискализацииЧека  
...
//
Процедура ПроверитьСведенияОРасчетеВИнтернет(Проверка, ДанныеЧека, СуммыОплат);     
	
	Если ДанныеЧека.Интернет Тогда
		
		Если ПустаяСтрока(ДанныеЧека.МестоРасчетов) Тогда
			ТекстОшибки = НСтр("ru = 'При расчете в сети Интернет необходимо указание места расчета – адрес сайта в сети Интернет.'");
			Проверка.Ошибки.Добавить(ТекстОшибки); 
		КонецЕсли;
		
		Если ПустаяСтрока(ДанныеЧека.ПокупательEmail) И ПустаяСтрока(ДанныеЧека.ПокупательНомер) Тогда     
			ТекстОшибки = НСтр("ru = 'При расчете в сети Интернет необходимо указание телефона или электронного адреса покупателя.'");
			Проверка.Ошибки.Добавить(ТекстОшибки); 
		КонецЕсли;       
		
		Если СуммыОплат.Наличные > 0 Тогда
			ТекстОшибки = НСтр("ru = 'При расчете в сети Интернет недопустимо указание наличных видов оплат.'");
			Проверка.Ошибки.Добавить(ТекстОшибки); 
		КонецЕсли;       
