...
Процедура ПодготовитьСтрокиВыписки(ДокументыКЗагрузке, БанковскийСчет, РеквизитыСчета, НастройкиЗагрузки) Экспорт
	...
	Отбор.Вставить("Загружать", Истина);
	СтрокиКЗагрузке = ДокументыКЗагрузке.НайтиСтроки(Отбор);
	ЗаполнитьСтатьюДвиженияДенежныхСредств(СтрокиКЗагрузке);
...
Процедура ЗаполнитьРасшифровкуСписания(СтрокаДокумента, ДокументОбъект, РеквизитыХозОперации, ПроводитьДокументы)
	...
	Если РеквизитыХозОперации.Найти("РасшифровкаПлатежа") <> Неопределено Тогда
		...
		Иначе
			...
			ДоступныеСтатьиДДС = ДенежныеСредстваПовтИсп.СтатьиХозяйственнойОперации(ДокументОбъект.ХозяйственнаяОперация);
...
Процедура ЗаполнитьСтатьюДвиженияДенежныхСредств(СтрокиКЗагрузке)
	...
	ВидыОпераций = ТаблицаКЗагрузке.Скопировать(, "ВидОперации");
	ВидыОпераций.Свернуть("ВидОперации");
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОперацииПлатежей.ВидОперации КАК ВидОперации
		|ПОМЕСТИТЬ ОперацииПлатежей
		|ИЗ
		|	&ВидыОпераций КАК ОперацииПлатежей
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Статьи.Ссылка КАК СтатьяДвиженияДенежныхСредств,
		|	ОперацииПлатежей.ВидОперации КАК ХозяйственнаяОперация
		|ИЗ
		...
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОперацииПлатежей КАК ОперацииПлатежей
		|			ПО ОперацииСтатей.ХозяйственнаяОперация = ОперацииПлатежей.ВидОперации
	...
	Для Каждого СтрокаТаблицы Из СтрокиКЗагрузке Цикл
		...
		Пока Выборка.НайтиСледующий(ПараметрыОтбора) Цикл
