...
Процедура ПрочитатьЗапросНаВыгрузку(КомпонентыОбмена, ОбъектXDTO)
	...
	// Чтобы понять, какие данные надо отправить повторно нужно:
	// - выбрать все ПКО, которые работают с указанным объектом формата.
	//   А поскольку мы не знаем, по какому правилу конвертации объект выгружался,
	//   то ссылку ищем по менеджерам всех ПКО.
	//
	...
	ОписаниеОтсутствующейСсылки = Новый Структура;
	ОписаниеОтсутствующейСсылки.Вставить("БлокирующийИдентификатор", Неопределено);
	ОписаниеОтсутствующейСсылки.Вставить("ВнешняяНавигационнаяСсылка", "");
	ОписаниеОтсутствующейСсылки.Вставить("ОбъектФормата", "");
	ОписаниеОтсутствующейСсылки.Вставить("СсылкаНайденаИЗарегистрированаКОтправке", Ложь);
	...
	ПравилаКонвертацииОтправка = ТаблицаПравилКонвертации(КомпонентыОбменаОтправка);
	Для каждого ОтсутствующаяСсылка Из ОбъектXDTO.Строка Цикл
		...
		ОписаниеОтсутствующейСсылки.БлокирующийИдентификатор = ОтсутствующаяСсылка["СсылкаЗапросаНаВыгрузку"];
		ОписаниеОтсутствующейСсылки.ВнешняяНавигационнаяСсылка = ОтсутствующаяСсылка["СсылкаВнешняяНавигационная"];
		ОписаниеОтсутствующейСсылки.ОбъектФормата = ОтсутствующаяСсылка["ИмяОбъектаФорматаПолученного"];
		ОписаниеОтсутствующейСсылки.СсылкаНайденаИЗарегистрированаКОтправке = Ложь;
		
		НайтиСсылкаНаОтправкуПоКоллекцииПравил(КомпонентыОбмена, ПравилаКонвертацииОтправка, ОписаниеОтсутствующейСсылки);
		Если ОписаниеОтсутствующейСсылки.СсылкаНайденаИЗарегистрированаКОтправке Тогда
			
			Продолжить;
			
		КонецЕсли;
		
		НайтиСсылкаНаОтправкуПоКоллекцииПравил(КомпонентыОбмена, КомпонентыОбмена.ПравилаКонвертацииОбъектов, ОписаниеОтсутствующейСсылки);
		Если НЕ ОписаниеОтсутствующейСсылки.СсылкаНайденаИЗарегистрированаКОтправке Тогда
			...
			ИдентификаторИзЗапроса = ОтсутствующаяСсылка["СсылкаЗапросаНаВыгрузку"];
			ИмяОбъектаФормата = ОтсутствующаяСсылка["ИмяОбъектаФорматаПолученного"];
...
Процедура НайтиСсылкаНаОтправкуПоКоллекцииПравил(КомпонентыОбмена, ПравилаКонвертации, ОписаниеОтсутствующейСсылки)
	
	ИдентификаторДляПоиска = Новый УникальныйИдентификатор(ОписаниеОтсутствующейСсылки.БлокирующийИдентификатор);
	...
	ОтборСтрокПКО.Вставить("ОбъектФормата", ОписаниеОтсутствующейСсылки.ОбъектФормата);
	...
	Для каждого ПравилоКонвертации Из НайденныеПКО Цикл
		...
		// У нас может быть несколько ПКО, 
		// которые относятся к разным таблицам метаданных
		// и платформа допускает, что в этих таблицах могут быть
		// ссылки с одинаковым идентификатором.
		ПодобраннаяОбъектнаяСсылка = МенеджерОбъектаМетаданных.ПолучитьСсылку(ИдентификаторДляПоиска);
		
		// В текущих обменах отсутствует событие КД "ПриОбработкеЗапросаВыгрузкиОбъекта"
	...
	РегистрироватьПодобранныеСсылки(КомпонентыОбмена, ОбъектыПовторнойВыгрузки, ОписаниеОтсутствующейСсылки);
	
КонецПроцедуры

Процедура РегистрироватьПодобранныеСсылки(КомпонентыОбмена, ОбъектыПовторнойВыгрузки, ОписаниеОтсутствующейСсылки)
	...
	Для каждого ПодобраннаяОбъектнаяСсылка Из ОбъектыПовторнойВыгрузки Цикл
		...
		ОтправкаЭлемента = ОтправкаЭлементаДанных.Авто;
		ОбменДаннымиСобытия.ПриОтправкеДанныхКорреспонденту(ПодобранныйОбъект, ОтправкаЭлемента, Ложь, УзелКорреспондента, Истина);
		
		Если ОтправкаЭлемента = ОтправкаЭлементаДанных.Удалить
			И ОбщегоНазначения.ЭтоРегистр(ПодобраннаяОбъектнаяСсылка.Метаданные())
			Тогда
		...
		Если ОтправкаЭлемента = ОтправкаЭлементаДанных.Авто Тогда
			...
			ОписаниеОтсутствующейСсылки.СсылкаНайденаИЗарегистрированаКОтправке = Истина;
			
		Иначе
			
			ОписаниеОшибки = НСтр("ru = 'Объект %1 не будет повторно выгружен, т.к. не соответствует условиям фильтра.'");
			ПредставлениеСсылки = ПредставлениеОбъектаДляПротокола(ОбъектыПовторнойВыгрузки);
			ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрШаблон(ОписаниеОшибки, ПредставлениеСсылки));
