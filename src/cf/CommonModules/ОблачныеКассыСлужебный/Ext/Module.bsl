...
//
Функция ДанныеЗапросаНаПробитиеЧека(ДокументОперации, ПараметрыПодключения, ДанныеЧека, ПараметрыПробитияЧека)
	...
	Исключение
		...
		Результат.КодОшибки         = ОблачныеКассыСервис.КодОшибкиНеверныйФорматЗапроса();
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось сериализовать данные чека в формат JSON:
				|%1
				|
				|Техническая информация об ошибке:
				|Документ операции: %2
