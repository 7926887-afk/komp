			...
			Если ТребуетсяОбновлениеКлючаСессии Тогда
				...
				Если Не ПараметрыЗапросаДобавленыКОбновлению(ПараметрыОбмена, ПараметрыЗапросаКлючаСессии, Сообщение) Тогда
					...
					Если СтрокаСертификата <> Неопределено Тогда
						...
						Если ПараметрыАвторизации.UnitedToken Тогда
							СертификатМЧД = ДанныеДляПроверкиСертификата[Организация];
							Если СертификатМЧД = Неопределено Тогда
								СертификатМЧД = ОбщегоНазначенияИС.ТребуетсяДоверенностьДляИспользованияСертификата(
									СтрокаСертификата.Сертификат, Организация, ПараметрыАвторизации.ИНН);
								ДанныеДляПроверкиСертификата.Вставить(Организация, СертификатМЧД);
							КонецЕсли;
							Если СертификатМЧД Тогда
								ПараметрыАвторизации.Данные = ПараметрыАвторизации.ИНН;
							КонецЕсли;
						КонецЕсли;
