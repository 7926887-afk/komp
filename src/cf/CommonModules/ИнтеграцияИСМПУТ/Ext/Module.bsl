...
Функция ОсобенностиУчетаНоменклатуры() Экспорт
	...
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.АвтозапчастиИКомплектующиеТранспортныхСредств);
	...
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.СпортивноеПитание);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.РастворимыеЗавариваемыеНапитки);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.ТоварыЛичнойГигиены);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.БритвыИЛезвия);
	Массив.Добавить(Перечисления.ОсобенностиУчетаНоменклатуры.НапиткиССодержаниемМолокаКакаоПрочие);
...
Процедура ОтразитьРезультатыПроверкиИПодбораВДокументеИнтеркампани(ПараметрыОкончанияПроверки, ТипОбъекта)
	...
	РазрешенОборотНемаркируемыхОстатков = ОбщегоНазначенияИСМППовтИсп.РазрешенОборотНемаркируемыхОстатковПродукции(ПараметрыОкончанияПроверки.ВидПродукцииИС, ДокументОбъект.Дата);
	Запрос.УстановитьПараметр("РазрешенОборотНемаркируемыхОстатков", РазрешенОборотНемаркируемыхОстатков);
	...
	|ИЗ
	|	Итог КАК Итог
	|ГДЕ НЕ &РазрешенОборотНемаркируемыхОстатков
...
Процедура ОтразитьИзменениеКоличестваВТабличнойЧастиТоварыКорректировкиРеализации(ДокументОбъект, ПараметрыОкончанияПроверки)
	...
	УпаковкиТоваров.Индексы.Добавить("Номенклатура,Характеристика");
	ВыборкаРасхождения = Пакет[5].Выбрать();
...
Процедура ЗаполнитьТабличныеЧастиВыводаИзОборотаИСМП(
	...
	Если (ДокументОбъект.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы
		Или ДокументОбъект.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы2025
		Или ДокументОбъект.ВидПродукции = Перечисления.ВидыПродукцииИС.СпортивноеПитание
...
Процедура ЗаполнитьВыводИзОборотаПриУведомленииНаОтгрузку(ДокументОбъект)
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВыводИзОборотаИСМП")
		И ИнтеграцияИСМПСлужебныйКлиентСервер.ДоступнаПередачаУведомленияОбЭкспорте(
			ДокументОбъект.ВидПродукции, ДокументОбъект.Операция) Тогда
