...
Функция ВременныеТаблицыДанныхРегистра(Запрос, ВозвращатьТаблицуВНаличии, ИмяТаблицыТовары = "ТоварыДляРаспределенияЗапасов", ИмяТаблицыКорректировки = "")
	...
	Текст =
		...
		|	Запасы.РезервироватьНаСкладеОстаток КАК РезервироватьНаСкладеПоВсемЗаказамСводно
		|ПОМЕСТИТЬ ВсеЗапасы
		|ИЗ
		|	РегистрНакопления.ЗапасыИПотребности.Остатки(,
		|		(Номенклатура, Характеристика, Склад, Назначение) В(
		...
		|	0 КАК РезервироватьНаСкладеПоВсемЗаказамСводно
		|ИЗ
		|	РегистрНакопления.ЗапасыИПотребности.Остатки(,
		|		(Номенклатура, Характеристика, Склад, Назначение) В(
		|			ВЫБРАТЬ
	...
	Если ИмяТаблицыКорректировки <> "" Тогда
		...
		Текст =
			...
			|	Корректировка.РезервироватьНаСкладе КАК РезервироватьНаСкладеПоВсемЗаказамСводно
			|ИЗ
			|	КорректировкаДвиженийПереопределяемый КАК Корректировка
			|ГДЕ
		...
		Текст =
			...
			|	СУММА(Набор.РезервироватьНаСкладеПоВсемЗаказамСводно) КАК РезервироватьНаСкладеПоВсемЗаказамСводно
			|ПОМЕСТИТЬ ВсеЗапасы
			|ИЗ
			|	НаборПереопределяемый КАК Набор
			|СГРУППИРОВАТЬ ПО
			...
			|ИМЕЮЩИЕ
			|	СУММА(Набор.Запас) <> 0
			|			ИЛИ СУММА(Набор.РезервироватьНаСкладеПоВсемЗаказамСводно) > 0";
		Текст = СтрЗаменить(Текст, "НаборПереопределяемый", СтрШаблон("(%1)", ТекстНабора));
	...
	Если ВозвращатьТаблицуВНаличии Тогда
		...
		Текст =
			...
			|	ЕСТЬNULL(Сведения.РезервироватьНаСкладеПоВсемЗаказамСводно, 0) КАК РезервироватьНаСкладеПоВсемЗаказамСводно,
			...
			|	Сведения.РезервироватьНаСкладеПоВсемЗаказамСводно КАК РезервироватьНаСкладеПоВсемЗаказамСводно,
			|	Сведения.Свободно КАК Свободно,
			|	Сведения.Излишек КАК Излишек,
			|	Сведения.Запас = 0 И Сведения.РезервироватьНаСкладеПоВсемЗаказамСводно = 0 И Сведения.Свободно = 0
			|		И Сведения.Излишек = 0 КАК Пустая
			|ИЗ
...
Процедура ВременнаяТаблицаЗапасыИДоступностьПоДатам(Запрос)
	...
	Запрос.Текст =
		...
		|	Запасы.РезервироватьНаСкладеПоВсемЗаказамСводно КАК РезервироватьНаСкладеПоВсемЗаказамСводно,
	...
	Для Индекс = 0 По ЗапасыИДоступностьПоДатам.Количество() - 1 Цикл
		...
		РаспределитьНаСтроку = Мин(РаспределитьВсего, 
			Макс(СтрокаТаблицы.Запас - СтрокаТаблицы.РезервироватьНаСкладеПоВсемЗаказамСводно, 0));
		СтрокаТаблицы.Свободно = СтрокаТаблицы.Запас
			- СтрокаТаблицы.РезервироватьНаСкладеПоВсемЗаказамСводно
			- РаспределитьНаСтроку;
	...
	Запрос.Текст =
		...
		|	ЗапасыИДоступностьПоДатам.РезервироватьНаСкладеПоВсемЗаказамСводно КАК РезервироватьНаСкладеПоВсемЗаказамСводно,
...
Процедура ВременнаяТаблицаРезультатРаспределенияЗапасовНаПотребности(Запрос, ИмяТаблицыТовары = "ТоварыДляРаспределенияЗапасов", ИмяТаблицыРезультат = "РаспределениеЗапасовНаПотребности", ИмяТаблицыРеестрДокументов = "РегистрСведений.РеестрДокументов") Экспорт
	...
	Текст =
		...
		|	СУММА(ВЫБОР
		|		КОГДА Запасы.Заказ = НЕОПРЕДЕЛЕНО ТОГДА
		|			Запасы.РезервироватьНаСкладеПоВсемЗаказамСводно
		|		ИНАЧЕ 0
		|	КОНЕЦ) КАК РезервироватьНаСкладеПоВсемЗаказамСводно,
		|	СУММА(ВЫБОР
		|		КОГДА Запасы.Заказ = НЕОПРЕДЕЛЕНО ТОГДА
		|			Запасы.Свободно
		|		ИНАЧЕ 0
		...
		|	ЕСТЬNULL(ЗапасыПоТоварам.ЗапасНаСкладе - ЗапасыПоТоварам.РезервироватьНаСкладеПоВсемЗаказамСводно, 0) КАК РаспределитьНаВсеСтроки,
		|	ЕСТЬNULL(ЗапасыПоТоварам.ЗапасНаСкладе, 0)
		|		- ЕСТЬNULL(ЗапасыПоТоварам.РезервироватьНаСкладеПоВсемЗаказамСводно, 0)
...
Функция ДанныеДляЗаписиТоваровВПроизводительномРежиме(Запрос, ИмяТаблицыТовары = "ТоварыДляРаспределенияЗапасов", ИмяТаблицыРезультат = "", ПолучатьПустые = Истина)
	...
	Текст =
		...
		|	Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно КАК РезервироватьНаСкладеПоВсемЗаказамСводно,
...
Процедура ДобавитьТекстЗапросаКонтроль(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	Если ТипДокумента = Тип("ДокументСсылка.ВозвратТоваровОтКлиента")
			...
			Или ТипДокумента = Тип("ДокументСсылка.ПоступлениеТоваровОтХранителя")
				И Не Документ.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5 Тогда
...
Функция ОшибкиПроверкиКорректностиЗаписейРегистраСведенийРаспределениеЗапасов() Экспорт
	...
	Текст =
		...
		|
		|/////////////////////////////////////////////////////////////
		// ресурсы РезервироватьНаСкладе - РезервироватьНаСкладеПоВсемЗаказамСводно - распределение запаса на все заказы
		|ВЫБРАТЬ
		|	Набор.Номенклатура КАК Номенклатура,
		|	Набор.Характеристика КАК Характеристика,
		|	Набор.Склад КАК Склад,
		...
		|		Таблица.Назначение КАК Назначение,
		|		0 КАК РезервироватьДвижения,
		|		Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно КАК РезервСведения
		|	ИЗ
		|		РегистрСведений.РаспределениеЗапасов КАК Таблица
		|	ГДЕ
		|		Таблица.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе)) КАК Набор
		...
		|
		|/////////////////////////////////////////////////////////////
		// ресурсы Свободно, Запас, РезервироватьНаСкладеПоВсемЗаказамСводно и Резервировать по мере поступления в регистре сведений
		|ВЫБРАТЬ
		|	Набор.Номенклатура КАК Номенклатура,
		|	Набор.Характеристика КАК Характеристика,
		|	Набор.Склад КАК Склад,
		...
		|					Таблица.Запас - Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно
		|				ИНАЧЕ
		|					0
		...
		|			ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемоеПоступление),
		|			ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ПоступлениеНеПодтвержденное))
		|		И Таблица.Свободно > 0 И ВсеЧтоДальше.Запас - ВсеЧтоДальше.РезервироватьНаСкладеПоВсемЗаказамСводно - ВсеЧтоДальше.Свободно > 0
		|;
		|
		|/////////////////////////////////////////////////////////////
		...
		|				ИЛИ Таблица.Зарезервировано <> 0 ИЛИ Таблица.Обеспечено <> 0 ИЛИ Таблица.НеОбеспечено <> 0 ИЛИ Таблица.Запас <> 0 ИЛИ Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно <> 0 ИЛИ Таблица.Свободно <> 0 ИЛИ Таблица.ВНаличии <> 0
		...
		|			Таблица.Запас = 0 И Таблица.Свободно = 0 И Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно = 0 И ВЫБОР КОГДА Таблица.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка) ТОГДА Таблица.ВНаличии = 0 ИНАЧЕ ИСТИНА КОНЕЦ
		...
		|				ИЛИ Таблица.ВНаличии <> 0 ИЛИ Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно <> 0 ИЛИ Таблица.Зарезервировано <> 0 ИЛИ Таблица.Обеспечено <> 0 ИЛИ Таблица.РезервироватьПоМереПоступления <> 0 ИЛИ Таблица.КОбеспечениюБезРезерва <> 0 ИЛИ Таблица.Резервировать <> 0 ИЛИ Таблица.НеОбеспечивать <> 0 ИЛИ Таблица.НеОбеспечено <> 0 ИЛИ Таблица.ОтложитьРезервирование <> 0
		|				ИЛИ Таблица.ЗаказНаОтгрузку <> НЕОПРЕДЕЛЕНО ИЛИ Таблица.ЖелаемаяДатаОтгрузки <> ДАТАВРЕМЯ(1,1,1) ИЛИ Таблица.ЗаказНаПоступление = НЕОПРЕДЕЛЕНО ИЛИ Таблица.ДатаПоступления = ДАТАВРЕМЯ(1,1,1) ИЛИ Таблица.ТипЗаписиРаспределенияЗапасов <> 1
		|		КОГДА Таблица.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ПоступлениеНеПодтвержденное) ТОГДА
		|			Таблица.Запас = 0 И Таблица.Свободно = 0
		|				ИЛИ Таблица.ВНаличии <> 0 ИЛИ Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно <> 0 ИЛИ Таблица.Зарезервировано <> 0 ИЛИ Таблица.Обеспечено <> 0 ИЛИ Таблица.КОбеспечениюБезРезерва <> 0 ИЛИ Таблица.РезервироватьПоМереПоступления <> 0 ИЛИ Таблица.Резервировать <> 0 ИЛИ Таблица.НеОбеспечивать <> 0 ИЛИ Таблица.НеОбеспечено <> 0 ИЛИ Таблица.ОтложитьРезервирование <> 0
		|				ИЛИ Таблица.ЗаказНаОтгрузку <> НЕОПРЕДЕЛЕНО ИЛИ Таблица.ЖелаемаяДатаОтгрузки <> ДАТАВРЕМЯ(1,1,1) ИЛИ Таблица.ЗаказНаПоступление = НЕОПРЕДЕЛЕНО ИЛИ Таблица.ДатаПоступления <> ДАТАВРЕМЯ(1,1,1) ИЛИ Таблица.ТипЗаписиРаспределенияЗапасов <> 1
		|		КОГДА Таблица.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ВРезерве) ТОГДА
		|			Таблица.Зарезервировано = 0
		|				ИЛИ Таблица.Обеспечено <> 0 ИЛИ Таблица.Запас <> 0 ИЛИ Таблица.Свободно <> 0 ИЛИ Таблица.ВНаличии <> 0 ИЛИ Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно <> 0 ИЛИ Таблица.РезервироватьПоМереПоступления <> 0 ИЛИ Таблица.КОбеспечениюБезРезерва <> 0 ИЛИ Таблица.Резервировать <> 0 ИЛИ Таблица.НеОбеспечивать <> 0 ИЛИ Таблица.НеОбеспечено <> 0 ИЛИ Таблица.ОтложитьРезервирование <> 0
		|				ИЛИ Таблица.ЗаказНаОтгрузку = НЕОПРЕДЕЛЕНО ИЛИ Таблица.ЗаказНаПоступление <> НЕОПРЕДЕЛЕНО ИЛИ Таблица.ДатаПоступления <> ДАТАВРЕМЯ(1,1,1) ИЛИ Таблица.ТипЗаписиРаспределенияЗапасов <> 1
		|		КОГДА Таблица.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.НеОбеспечивать) ТОГДА
		|			Таблица.НеОбеспечено = 0
		|				ИЛИ Таблица.Запас <> 0 ИЛИ Таблица.Свободно <> 0 ИЛИ Таблица.ВНаличии <> 0 ИЛИ Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно <> 0 ИЛИ Таблица.Зарезервировано <> 0 ИЛИ Таблица.Обеспечено <> 0 ИЛИ Таблица.РезервироватьПоМереПоступления <> 0 ИЛИ Таблица.КОбеспечениюБезРезерва <> 0 ИЛИ Таблица.Резервировать <> 0 ИЛИ Таблица.НеОбеспечивать <> 0 ИЛИ Таблица.ОтложитьРезервирование <> 0
		|				ИЛИ Таблица.ЗаказНаОтгрузку = НЕОПРЕДЕЛЕНО ИЛИ Таблица.ЗаказНаПоступление <> НЕОПРЕДЕЛЕНО ИЛИ Таблица.ДатаПоступления <> ДАТАВРЕМЯ(1,1,1) ИЛИ Таблица.ТипЗаписиРаспределенияЗапасов <> 1
		...
		|				ИЛИ Таблица.Запас <> 0 ИЛИ Таблица.Свободно <> 0 ИЛИ Таблица.ВНаличии <> 0 ИЛИ Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно <> 0 ИЛИ Таблица.КОбеспечениюБезРезерва <> 0 ИЛИ Таблица.Резервировать <> 0 ИЛИ Таблица.НеОбеспечивать <> 0 ИЛИ Таблица.НеОбеспечено <> 0 ИЛИ Таблица.ОтложитьРезервирование <> 0
		...
		|		Таблица.Склад КАК Склад,
		|		Таблица.Назначение КАК Назначение,
		|		Таблица.Запас - Таблица.Свободно - Таблица.РезервироватьНаСкладеПоВсемЗаказамСводно КАК РаспределеноДвижения,
