...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюЗаполнениеИдентификаторовЗаписей(Параметры) Экспорт
	...
	Попытка
		...
		Если Не РезультатыЗапроса[0].Пустой() Тогда
			...
			НаборЗаписей.ДополнительныеСвойства.Вставить("УдалениеЗаписей", Истина);
			НаборЗаписей.Записать(РежимЗамещения.Удаление);
		КонецЕсли;

		Если Не РезультатыЗапроса[1].Пустой() Тогда
			ДанныеРегистра = РезультатыЗапроса[1].Выгрузить();
			Для Каждого ЗаписьРегистра Из ДанныеРегистра Цикл
				Если Не ЗначениеЗаполнено(ЗаписьРегистра.ИдентификаторЗаписи) Тогда
					ЗаписьРегистра.ИдентификаторЗаписи = Строка(Новый УникальныйИдентификатор);
				КонецЕсли;
			КонецЦикла;
			НаборЗаписей = СоздатьНаборЗаписей();
