...
Процедура ЗавершитьВвод(Отказ=Ложь, ЗакрытьФорму=Ложь)
	...
	Если Объект.РезультатВзятьИзПула Тогда
		...
		Если ИнтеграцияИСКлиентСервер.ДоступенСпособВводаВОборотПеремаркировка(Объект.ВидПродукции) Тогда
			СтруктураПечатиЭтикетки.СпособВводаВОборот = ПредопределенноеЗначение(
				"Перечисление.СпособыВводаВОборотСУЗ.Перемаркировка");
		Иначе
			СтруктураПечатиЭтикетки.СпособВводаВОборот = ПредопределенноеЗначение(
				"Перечисление.СпособыВводаВОборотСУЗ.МаркировкаВнеПроизводстваИмпорта");
		КонецЕсли;
