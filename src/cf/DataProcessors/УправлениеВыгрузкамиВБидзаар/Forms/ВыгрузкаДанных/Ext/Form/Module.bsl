			...
			Если Параметры.Свойство("ИмяКоманды", ИмяКоманды) Тогда
				...
				ИначеЕсли ИмяКоманды = "УдалитьПоОтбору" Тогда
					...
					Исключение
						...
						ТекстИсключения = СтрШаблон(ТекстИсключения, "УправлениеВыгрузкамиВБидзаар.УдалитьПоОтбору");
...
Процедура УдалитьВсеСтрокиНаСервере()
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстИсключения = НСтр("ru = '%1'", ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстИсключения = СтрШаблон(ТекстИсключения, "УправлениеВыгрузкамиВБидзаар.ДанныеВыгрузкиПередУдалением");
