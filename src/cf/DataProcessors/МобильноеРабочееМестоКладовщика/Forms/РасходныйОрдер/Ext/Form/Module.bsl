...
Процедура ЗакончитьОтгрузку(Команда)
	...
	Если ТоварыКОтгрузке.Количество() > 0 Или ТоварыОтобрано.Количество() > 0 Тогда
		Если ИспользоватьАдресноеХранение Тогда
			ПоказатьВопрос(Оповещение, НСтр("ru = 'Завершить отгрузку? Товары, оставшиеся на вкладках «Отобрать» и «Отобрано» не будут отгружены.'"), СписокЗначений, 0);
		Иначе
			ПоказатьВопрос(Оповещение, НСтр("ru = 'Завершить отгрузку? Товары, оставшиеся на вкладке «К отгрузке» не будут отгружены.'"), СписокЗначений, 0);
		КонецЕсли;
	Иначе
...
Процедура ЗаполнитьФорму(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Результат <> Неопределено И Результат.Свойство("ШтрихкодыУпаковок") Тогда
