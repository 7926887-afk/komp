...
Процедура УстановитьПоляПредставленияСтроки(Форма, ТекущиеДанные)
	...
	ЭтоБАД                 = (СтрокаДанных["ВидПродукции"] = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы")
		...
		Или СтрокаДанных["ВидПродукции"] = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.СпортивноеПитание"));
	...
	Если ЗначениеЗаполнено(СтрокаДанных["Статус"]) Тогда
		...
		Если СтрокаДанных["ОсобоеСостояние"] = ПредопределенноеЗначение("Перечисление.СтатусыКодовМаркировкиИСМП.ЗаблокированОГВ")
			Или СтрокаДанных["ОсобоеСостояние"] = ПредопределенноеЗначение("Перечисление.СтатусыКодовМаркировкиМОТП.ЗаблокированОГВ") Тогда
...
Процедура Подключаемый_ПолученыДанныеИзТСД(Штрихкоды, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЗагрузкаДанныхТСД.ОтсутствуютCDNПлощадки Тогда
		...
		ПараметрыЗапроса.Параметры               = Новый Структура("НеВыводитьОкноОжидания, НомерФискальногоНакопителя", Ложь, ПараметрыСканирования.НомерФискальногоНакопителя);
