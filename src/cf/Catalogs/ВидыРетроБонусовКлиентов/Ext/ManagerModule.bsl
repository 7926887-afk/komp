...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	Данные.БезРасчета
	|	И (Данные.БазаРасчетаПродаж <> ЗНАЧЕНИЕ(Перечисление.БазыРасчетаРетроБонусов.ПустаяСсылка)
			...
			Если СправочникОбъект <> Неопределено Тогда
				...
				Если СправочникОбъект.БезРасчета Тогда
					...
					Если НЕ СправочникОбъект.БазаРасчетаПродаж.Пустая() Тогда
						...
						СправочникОбъект.БазаРасчетаПродаж = Перечисления.БазыРасчетаРетроБонусов.ПустаяСсылка();
...
Функция БазовыйОтборТоваров(МодульНастройки)
	...
	МодульНастройки.НовыйОтбор(Отбор, "ПоказательТоваров", ИсклПоказатели,, ВидСравненияКомпоновкиДанных.НеВСписке);
