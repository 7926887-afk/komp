...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Попытка
		...
		|ИЗ
		|	РегистрНакопления.РаспоряженияНаОтгрузку.Обороты(&НачалоПериода,,, Распоряжение В (&Распоряжения)) КАК
		...
		Если ЗначениеЗаполнено(МинимальнаяДатаРаспоряжений) Тогда
...
Функция МинимальнаяДатаРаспоряжений(Распоряжения)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(РаспоряженияНаОтгрузку.Период, ДЕНЬ)) КАК Дата
	|ИЗ
	|	РегистрНакопления.РаспоряженияНаОтгрузку КАК РаспоряженияНаОтгрузку
	|ГДЕ
