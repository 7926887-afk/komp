...
Процедура ПоместитьВременнуюТаблицуТоваров_Дополнительный(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	|	ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка) КАК Упаковка,
	|" + Кт_ОбщегоНазначенияСервер.ТекстЗапросаСодержаниеПоНоменклатуреПартнера("ТаблицаДокумента") + " КАК Содержание
	...
	|" + Кт_ОбщегоНазначенияСервер.ТекстЗапросаСодержаниеПоНоменклатуреПартнера("СтрокиТоваров") + " КАК Содержание
	|ИЗ
