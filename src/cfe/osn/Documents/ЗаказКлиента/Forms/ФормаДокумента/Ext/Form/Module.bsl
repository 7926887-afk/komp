...
&НаСервере
Функция Кт_ДоговорОбязателенНаФорме()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи
		...
		Или (КомиссионныеПродажи25 И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Если ИспользоватьСоглашенияСКлиентами Тогда
		...
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Соглашение, "ИспользуютсяДоговорыКонтрагентов");
		
	Иначе
		
		Возврат ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыСКлиентами");
...
&НаСервере
Процедура Кт_УстановитьОтметкуНезаполненногоДоговораНаФорме()
	
	ДоговорОбязателен = Кт_ДоговорОбязателенНаФорме();
	ИменаЭлементов = Новый Массив;
	ИменаЭлементов.Добавить("Договор");
	ИменаЭлементов.Добавить("Договор1");
	
	Для Каждого ИмяЭлемента Из ИменаЭлементов Цикл
		
		ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, ИмяЭлемента, "АвтоОтметкаНезаполненного", ДоговорОбязателен);
		Если Не ДоговорОбязателен Тогда
			ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, ИмяЭлемента, "ОтметкаНезаполненного", Ложь);
		КонецЕсли;
...
Процедура Кт_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Кт_УстановитьОтметкуНезаполненногоДоговораНаФорме();
	
КонецПроцедуры

&НаСервере
&После("ПриЧтенииНаСервере")
Процедура Кт_ПриЧтенииНаСервереПосле(ТекущийОбъект)
...
Процедура Кт_ХозяйственнаяОперацияПриИзмененииСерверПосле(ВызовПоКоманде)
...
Процедура Кт_ПартнерПриИзмененииСерверПосле()
...
Процедура Кт_КонтрагентПриИзмененииСерверПосле()
