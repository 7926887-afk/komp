...
Процедура Кт_ПроверитьВозможностьВводаНаОсновании(ЗаказКлиента, Статус = Неопределено, ЕстьОшибкиПроведен = Ложь, КВыполнению = Ложь) Экспорт
...
#Вставка
	Если НЕ _ОбработкаЗакупкиСервер.ЕстьОплатаПоЗаказуКлиента(ЗаказКлиента) Тогда
		
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'По заказу клиента %1 отсутствует полная оплата. Ввод заявки на закупку невозможен.'"),
			ЗаказКлиента);
...
Функция Кт_ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов, КодЯзыка = Неопределено) Экспорт
	
	// Добавим в запрос номенклатуру контрагента в Содержание
	...
	|		ИНАЧЕ Таблица.НоменклатураПартнера.НаименованиеПолное
