...
Функция ТекстЗапросаРеквизитыДоставки() Экспорт
	...
	|ИЗ
	|	Документ.ЗаявкаНаЗакупку.Товары КАК ДанныеДокумента
	|ГДЕ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаЗакупку КАК ИсточникДанныхДоставки
...
Функция ТекстЗапросаТоварыКДоставке() Экспорт
	...
	|ИЗ
	|	Документ.ЗаявкаНаЗакупку.Товары КАК Т
	|ГДЕ
	|	Т.Ссылка В(&Ссылки)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.ЗаявкаНаЗакупку) Тогда
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Менеджер = Метаданные.Документы.ЗаявкаНаЗакупку.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление = ОбщегоНазначенияУТ.ПредставлениеОбъекта(Метаданные.Документы.ЗаявкаНаЗакупку);
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
...
Функция ДобавитьКомандуСоздатьОбеспечивающийЗаказ(КомандыСозданияНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.ЗаявкаНаЗакупку) Тогда
		...
		ПараметрыСоздания.ИмяМенеджераДокумента = "ЗаявкаНаЗакупку";
		ПараметрыСоздания.ИмяФормы = "Документ.ЗаявкаНаЗакупку.ФормаОбъекта";
...
Функция РеквизитыДокумента(ДокументСсылка) Экспорт
	...
	|	И ТаблицаТовары.ЗаявкаНаЗакупку = &ДокументСсылка
	...
	|ИЗ
	|	Документ.ЗаявкаНаЗакупку КАК ДанныеДокумента
...
Функция СформироватьЗапросПроверкиПриСменеСтатуса(МассивДокументов, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры <> Неопределено И ДополнительныеПараметры.Свойство("КонтрольВыполненияЗаказа") Тогда
		...
		//-- Локализация
		|	ВЫБОР КОГДА втОстаткиПоЗаказам.ЗаявкаНаЗакупку ЕСТЬ NULL
		|		ТОГДА ЛОЖЬ
		|			ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК ЕстьОстаткиПоЗаказу
		|ИЗ
		|	Документ.ЗаявкаНаЗакупку КАК ТаблицаДокументов
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	втОстаткиПоЗаказам
		|ПО
		|	ТаблицаДокументов.Ссылка = втОстаткиПоЗаказам.ЗаявкаНаЗакупку
		//++ Локализация
		|ЛЕВОЕ СОЕДИНЕНИЕ 
...
Функция ПравоДоступаДобавление() Экспорт
	...
	Возврат ПравоДоступа("Добавление", Метаданные.Документы.ЗаявкаНаЗакупку);
...
//
// Параметры:
//	ЗаявкаНаЗакупку         - ДокументСсылка.ЗаявкаНаЗакупку - Документ, состояние которого необходимо вычислить
...
Процедура РассчитатьСостояние(
	...
	Если ЗначениеЗаполнено(ЗаявкаНаЗакупку) И ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РасчетыСПоставщиками) Тогда
		...
		ТекстЗапроса = "
			|ВЫБРАТЬ
			|	ВЫБОР
			|		КОГДА (НЕ ДокументЗаявкаНаЗакупку.Проведен)
			...
			|		КОГДА (НЕ ДокументЗаявкаНаЗакупку.Проведен)
			|			ТОГДА ЛОЖЬ
			|		КОГДА (ДокументЗаявкаНаЗакупку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Закрыт) И &ИспользоватьСтатусы)
			|			ТОГДА ЛОЖЬ
			...
			|ВЫБОР
			|	КОГДА
			|		ДокументЗаявкаНаЗакупку.Проведен И
			|		((ДокументЗаявкаНаЗакупку.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.НеСогласован) И
			|		ДокументЗаявкаНаЗакупку.СуммаДокумента > 0)
			|		ИЛИ НЕ (ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам)
			|			ИЛИ ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)))
			|	ТОГДА
			...
			|ВЫБОР
			|	КОГДА
			|		ДокументЗаявкаНаЗакупку.Проведен
			|		И (ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов)
			|			ИЛИ ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамНакладным))
			|	ТОГДА
			...
			|ИЗ
			|	Документ.ЗаявкаНаЗакупку КАК ДокументЗаявкаНаЗакупку
			|ЛЕВОЕ СОЕДИНЕНИЕ
			...
			|ПО
			|	СостоянияЗаказовПоставщикам.Заказ = ДокументЗаявкаНаЗакупку.Ссылка
			|ГДЕ
			|	ДокументЗаявкаНаЗакупку.Ссылка = &ЗаявкаНаЗакупку
		...
		Если ПроверитьЗаданияКОтражениюСостоянияЗаказов Тогда
			...
			ТекстЗапросаЕстьЗаданияКОтражениюСостоянияЗаказов =
				СостоянияДокументов.ТекстЗапросаЕстьЗаданияКОтражениюСостоянияЗаказов("&ЗаявкаНаЗакупку");
		...
		ОбъектРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаявкаНаЗакупку, "ОбъектРасчетов");
...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса = "
		|ВЫБРАТЬ
		|	ЗаказыОстатки.ЗаявкаНаЗакупку                        КАК ЗаявкаНаЗакупку,
		...
		|
		|ИЗ (ВЫБРАТЬ
		|		ЗаказыПоставщикамОстатки.ЗаявкаНаЗакупку КАК ЗаявкаНаЗакупку,
		...
		|		ЗаявкаНаЗакупку В (&МассивЗаказов)) КАК ЗаказыПоставщикамОстатки) КАК ЗаказыОстатки
		|СГРУППИРОВАТЬ ПО
		|	ЗаказыОстатки.ЗаявкаНаЗакупку
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|// НЕ ПРОВЕДЕН /////////////////////////////////////////////////////////////
		|	КОГДА
		|		НЕ ДокументЗаявкаНаЗакупку.Проведен
		|	ТОГДА
		|		ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовПоставщикам.ПустаяСсылка)
		|// ОЖИДАЕТСЯ СОГЛАСОВАНИЕ //////////////////////////////////////////////////
		|	КОГДА
		|		ДокументЗаявкаНаЗакупку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.НеСогласован)
		|	ТОГДА
		|		ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовПоставщикам.ОжидаетсяСогласование)
		|// ОЖИДАЕТСЯ АВАНС ДО ПОДТВЕРЖДЕНИЯ /////////////////////////////////////////
		|	КОГДА
		|		(ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам)
		|			ИЛИ ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным))
		|		И ДокументЗаявкаНаЗакупку.СуммаАвансаДоПодтверждения > 0
		|		И ЕСТЬNULL(СостояниеВзаиморасчетов.ОплаченоПоЗаказу, 0) < ДокументЗаявкаНаЗакупку.СуммаАвансаДоПодтверждения
		...
		|	КОГДА
		|		ДокументЗаявкаНаЗакупку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Согласован)
		|		И ДокументЗаявкаНаЗакупку.СуммаДокумента > 0
		|		И (ЕСТЬNULL(СостояниеВзаиморасчетов.ОплаченоПоЗаказу, 0) >= ДокументЗаявкаНаЗакупку.СуммаАвансаДоПодтверждения
		|			ИЛИ НЕ ДокументЗаявкаНаЗакупку.ПорядокРасчетов В (ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам), ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)))
		|	ТОГДА
		...
		|		(ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам)
		|			ИЛИ ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным))
		|		И (ДокументЗаявкаНаЗакупку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Согласован) ИЛИ &НеИспользоватьСтатусыЗаказовПоставщикам
		|		ИЛИ ДокументЗаявкаНаЗакупку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден))
		|		И ДокументЗаявкаНаЗакупку.СуммаПредоплатыДоПоступления > 0
		|		И ЕСТЬNULL(СостояниеВзаиморасчетов.ОплаченоПоЗаказу, 0) < ДокументЗаявкаНаЗакупку.СуммаПредоплатыДоПоступления + ДокументЗаявкаНаЗакупку.СуммаАвансаДоПодтверждения
		|		И (ЕСТЬNULL(СостояниеВзаиморасчетов.ОсталосьОплатить, 0) < 0 ИЛИ СостояниеВзаиморасчетов.Заказ ЕСТЬ NULL)
		|	ТОГДА
		|		ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовПоставщикам.ОжидаетсяПредоплатаДоПоступления)
		|// ГОТОВ К ПОСТУПЛЕНИЮ //////////////////////////////////////////////////////
		|	КОГДА
		|		ДокументЗаявкаНаЗакупку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден)
		|		И ДокументЗаявкаНаЗакупку.СуммаДокумента > 0
		|		И (ЕСТЬNULL(СостояниеВзаиморасчетов.ОплаченоПоЗаказу, 0) >= ДокументЗаявкаНаЗакупку.СуммаПредоплатыДоПоступления + ДокументЗаявкаНаЗакупку.СуммаАвансаДоПодтверждения
		|			ИЛИ НЕ ДокументЗаявкаНаЗакупку.ПорядокРасчетов В (ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам), ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)))
		...
		|	КОГДА
		|		(ДокументЗаявкаНаЗакупку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден) ИЛИ &НеИспользоватьСтатусыЗаказовПоставщикам)
		|		И ДокументЗаявкаНаЗакупку.СуммаДокумента > 0
		|		И (ЕСТЬNULL(СостояниеВзаиморасчетов.ОплаченоПоЗаказу, 0) >= ДокументЗаявкаНаЗакупку.СуммаПредоплатыДоПоступления + ДокументЗаявкаНаЗакупку.СуммаАвансаДоПодтверждения
		|			ИЛИ НЕ ДокументЗаявкаНаЗакупку.ПорядокРасчетов В (ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам), ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)))
		...
		|		(ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам)
		|			ИЛИ ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным))
		|		И (ДокументЗаявкаНаЗакупку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден) ИЛИ &НеИспользоватьСтатусыЗаказовПоставщикам)
		|		И ДокументЗаявкаНаЗакупку.СуммаДокумента - ДокументЗаявкаНаЗакупку.СуммаАвансаДоПодтверждения - ДокументЗаявкаНаЗакупку.СуммаПредоплатыДоПоступления > 0
		...
		|// ГОТОВ К ЗАКРЫТИЮ ////////////////////////////////////////////////////////
		|	КОГДА
		|		ДокументЗаявкаНаЗакупку.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Закрыт) И &КонтролироватьЗакрытиеЗаказа
		|	ТОГДА
		|		ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовПоставщикам.ГотовКЗакрытию)
		|// ЗАКРЫТ //////////////////////////////////////////////////////////////////
		|	КОГДА
		|		ДокументЗаявкаНаЗакупку.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Закрыт),ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден))
		|	ТОГДА
		...
		|		ДокументЗаявкаНаЗакупку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.НеСогласован)
		|	ТОГДА
		|		ДокументЗаявкаНаЗакупку.ДатаСогласования
		|// ДАТА АВАНСА /////////////////////////////////////////////////////////////
		|	КОГДА
		|		(ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам)
		|			ИЛИ ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным))
		...
		|			ИЛИ НЕ ДокументЗаявкаНаЗакупку.ПорядокРасчетов В (ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам), ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)))
		|	ТОГДА
		|		ДокументЗаявкаНаЗакупку.ДатаПервогоПоступления
		|// ДАТА ПРЕДОПЛАТЫ /////////////////////////////////////////////////////////
		|	КОГДА
		|		(ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам)
		|			ИЛИ ДокументЗаявкаНаЗакупку.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным))
		...
		|	ТОГДА
		|		
		|		ЕСТЬNULL(ДатыПоступленияЗаказовПоставщикам.МинимальнаяДатаПоступления, ДокументЗаявкаНаЗакупку.ДатаПервогоПоступления)
		...
		|// СУММА ОПЛАТЫ /////////////////////////////////////////////////////////////
		|ВЫБОР
		|	КОГДА ДокументЗаявкаНаЗакупку.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПриемНаКомиссию)
		...
		|	КОГДА ДокументЗаявкаНаЗакупку.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПриемНаКомиссию)
		|			И ДокументЗаявкаНаЗакупку.СуммаДокумента > 0
		|		ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ОплаченоПоЗаказам.КПоступлениюРасход, 0) * 100 / ДокументЗаявкаНаЗакупку.СуммаДокумента КАК ЧИСЛО(20, 10))
		...
		|	ДокументЗаявкаНаЗакупку.Ссылка КАК Заказ
		|ИЗ
		|	Документ.ЗаявкаНаЗакупку КАК ДокументЗаявкаНаЗакупку
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	ВтСостояниеВзаиморасчетов КАК СостояниеВзаиморасчетов
		|ПО
		|	ДокументЗаявкаНаЗакупку.Ссылка = СостояниеВзаиморасчетов.Заказ
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	ОплаченоПоЗаказам КАК ОплаченоПоЗаказам
		|ПО
		|	ДокументЗаявкаНаЗакупку.Ссылка = ОплаченоПоЗаказам.ЗаявкаНаЗакупку
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	ВТЗаказыПоставщикамОстатки КАК ЗаказыПоставщикамОстатки
		|ПО
		|	ДокументЗаявкаНаЗакупку.Ссылка = ЗаказыПоставщикамОстатки.ЗаявкаНаЗакупку
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	ВТТоварыКПоступлениюОстатки КАК ТоварыКПоступлениюОстатки
		|ПО
		|	ДокументЗаявкаНаЗакупку.Ссылка = ТоварыКПоступлениюОстатки.ДокументПоступления
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	ДатыАктуальностиЗаказовПоставщикам КАК ДатыАктуальностиРасчетов
		|ПО
		|	ДокументЗаявкаНаЗакупку.Ссылка = ДатыАктуальностиРасчетов.ОбъектРасчетов
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	ДатыПоступленияЗаказовПоставщикам КАК ДатыПоступленияЗаказовПоставщикам
		|ПО
		|	ДокументЗаявкаНаЗакупку.Ссылка = ДатыПоступленияЗаказовПоставщикам.ЗаявкаНаЗакупку
		|ГДЕ
		|	ДокументЗаявкаНаЗакупку.Ссылка В (&МассивЗаказов)
		|
		|";
	Возврат ТекстЗапроса;
...
//
// Параметры:
//  Объект - ДокументОбъект.ЗаявкаНаЗакупку - документ, по которому необходимо сформировать параметры.
//
// Возвращаемое значение:
...
//
// Параметры:
//  Объект - ДокументОбъект.ЗаявкаНаЗакупку - 
//
// Возвращаемое значение:
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЗаявкаНаЗакупку.Дата                         КАК Период,
	|	ЗаявкаНаЗакупку.Валюта                       КАК Валюта,
	|	ЗаявкаНаЗакупку.ВалютаСоглашения             КАК ВалютаСоглашения,
	|	ЗаявкаНаЗакупку.ЦенаВключаетНДС              КАК ЦенаВключаетНДС,
	|	ЗаявкаНаЗакупку.ЦенаВключаетНДССоглашения    КАК ЦенаВключаетНДССоглашения,
	|	ЗаявкаНаЗакупку.Статус                       КАК Статус,
	|	ЗаявкаНаЗакупку.Партнер                      КАК Партнер,
	...
	|	ЗаявкаНаЗакупку.Соглашение                   КАК Соглашение,
	|	ЗаявкаНаЗакупку.РегистрироватьЦеныПоставщика КАК РегистрироватьЦеныПоставщика,
	|	ЗаявкаНаЗакупку.ВариантПриемкиТоваров        КАК ВариантПриемкиТоваров,
	|	ЗаявкаНаЗакупку.Договор                      КАК Договор,
	|	ЗаявкаНаЗакупку.ДоговорПредставление         КАК ДоговорПредставление,
	|	ЗаявкаНаЗакупку.ВернутьМногооборотнуюТару    КАК ВернутьМногооборотнуюТару,
	|	ЗаявкаНаЗакупку.ТребуетсяЗалогЗаТару         КАК ТребуетсяЗалогЗаТару,
	|	ЗаявкаНаЗакупку.НаправлениеДеятельности      КАК НаправлениеДеятельности,
	|	ЗаявкаНаЗакупку.Склад                        КАК Склад,
	|	ЗаявкаНаЗакупку.Номер                        КАК Номер,
	|	ЗаявкаНаЗакупку.Менеджер                     КАК Менеджер,
	|	ЗаявкаНаЗакупку.Автор                        КАК Автор,
	|	ЗаявкаНаЗакупку.Комментарий                  КАК Комментарий,
	|	ЗаявкаНаЗакупку.СуммаДокумента               КАК СуммаДокумента,
	|	ЗаявкаНаЗакупку.Проведен                     КАК Проведен,
	|	ЗаявкаНаЗакупку.ПометкаУдаления              КАК ПометкаУдаления,
	|	ЗаявкаНаЗакупку.ДатаПоДаннымПоставщика       КАК ДатаПоДаннымПоставщика,
	|	ЗаявкаНаЗакупку.НомерПоДаннымПоставщика      КАК НомерПоДаннымПоставщика,
	|	ЗаявкаНаЗакупку.Подразделение                КАК Подразделение,
	|	ЗаявкаНаЗакупку.Приоритет                    КАК Приоритет
	|
	|ИЗ	ДанныеДокументаШапка КАК ЗаявкаНаЗакупку";
	...
	Запрос.УстановитьПараметр("ЗаявкаНаЗакупку",              ДокументСсылка);
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	|ИЗ
	|	Документ.ЗаявкаНаЗакупку КАК ДанныеШапки
	|ГДЕ
...
Процедура ТекстЗапросаДанныеДокументаТаблицаЭтапыГрафикаОплаты(СписокЗапросов)
	...
	|ИЗ
	|	Документ.ЗаявкаНаЗакупку.ЭтапыГрафикаОплаты КАК ТаблицаЭтапыГрафикаОплаты
	|ГДЕ
...
Процедура СформироватьСуммыДокументаВВалютахУчета(Запрос, ТекстыЗапроса, Регистры = Неопределено, ДокументОбъект = Неопределено) Экспорт
	...
	Если ДокументОбъект = Неопределено Тогда
		...
		ТекстЗапросаДанных = СтрЗаменить(ТекстЗапросаДанных, "ДанныеДокументаТаблицаТовары", "Документ.ЗаявкаНаЗакупку.Товары");
		ТекстЗапросаДанных = СтрЗаменить(ТекстЗапросаДанных, "ДанныеДокументаШапка", "Документ.ЗаявкаНаЗакупку");
...
Функция ТекстЗапросаТаблицаЗаказыПоставщикам(Запрос, ТекстыЗапроса, Регистры, ДокументОбъект = Неопределено)
	...
	|	&Период                                                           КАК Период,
	|	ТаблицаТовары.Ссылка                                              КАК ЗаявкаНаЗакупку,
	|	ТаблицаТовары.Номенклатура                                        КАК Номенклатура,
	|	ТаблицаТовары.Характеристика                                      КАК Характеристика,
	|	ТаблицаТовары.КодСтроки                                           КАК КодСтроки,
	...
	Если ДокументОбъект = Неопределено Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДанныеДокументаТаблицаТовары", "Документ.ЗаявкаНаЗакупку.Товары");
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	ПолноеИмяДокумента      = "Документ.ЗаявкаНаЗакупку";
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//КомандаПечати.ДополнительныеПараметры.Вставить("Тип", "ЗаявкаНаЗакупку");
	...
	//	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	//	КомандаПечати.ДополнительныеПараметры.Вставить("Тип", "ЗаявкаНаЗакупкуПоДаннымПоставщика");
	//	
	//КонецЕсли;
...
Функция ПолучитьДанныеДляПечатнойФормыЗаказаНаТоварыУслуги(МассивОбъектов, ПараметрыПечати, КодЯзыка = Неопределено) Экспорт
	...
	|
	|ИЗ
	|	Документ.ЗаявкаНаЗакупку КАК Документы
	|
	|ГДЕ
	...
	Запрос.УстановитьПараметр("ПредставлениеВОшибке",	Метаданные.Документы.ЗаявкаНаЗакупку.Представление());
...
Функция ТекстЗапросаТоварыДокумента(Отбор) Экспорт
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	Таблица.Ссылка                       КАК ЗаявкаНаЗакупку,
	...
	|ИЗ
	|	Документ.ЗаявкаНаЗакупку.Товары КАК Таблица
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтДанныеУчета КАК ДанныеУчета
	|	ПО Таблица.Ссылка = ДанныеУчета.ЗаявкаНаЗакупку
	|	И Таблица.КодСтроки = ДанныеУчета.КодСтроки
	|ГДЕ
	|	НЕ ДанныеУчета.ЗаявкаНаЗакупку ЕСТЬ NULL
	|	И &Отбор";
...
Функция СуммыПоЗаказам(СсылкаОбъект, ЗначенияРеквизитов = Неопределено) Экспорт
	...
	Если ТипЗнч(СсылкаОбъект) = Тип("ДокументСсылка.ЗаявкаНаЗакупку") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Таблица", "Документ.ЗаявкаНаЗакупку.Товары");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ИмяФормы = "Документ.ЗаявкаНаЗакупку.Форма.ФормаСпискаДокументов";
	...
	Доступность =
		(ОбщиеПараметрыЗапросов.ЭтоПолноправныйПользователь
			Или ПравоДоступа("Просмотр", Метаданные.Документы.ЗаявкаНаЗакупку))
		И ПравоДоступа("Добавление", Метаданные.Документы.ЗаявкаНаЗакупку)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДокументЗаявкаНаЗакупку.Ссылка) КАК ЗаказыПоставщикамВсегоВРаботе,
	...
	|		ПО (СостоянияЗаказовПоставщикам.Заказ = ДокументЗаявкаНаЗакупку.Ссылка)}
	|ГДЕ
	|	(НЕ (СостоянияЗаказовПоставщикам.Заказ ЕСТЬ NULL) ИЛИ НЕ ДокументЗаявкаНаЗакупку.Проведен)
	|	И ДокументЗаявкаНаЗакупку.Менеджер = &Пользователь
	|	И СостоянияЗаказовПоставщикам.Состояние <> ЗНАЧЕНИЕ(Перечисление.СостоянияЗаказовПоставщикам.Закрыт)
	|	И (НЕ ДокументЗаявкаНаЗакупку.ПометкаУдаления)";
...
//
// Параметры:
//  ЗаявкаНаЗакупку	 - ДокументСсылка.ЗаявкаНаЗакупку - Проверяемый заказ.
//
// Возвращаемое значение:
//  Булево - Результат проверки
//
Функция ЗаказЯвляетсяРаспоряжениемНаПриемкуТоваров(ЗаявкаНаЗакупку) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ВариантПриемкиТоваровЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаявкаНаЗакупку, "ВариантПриемкиТоваров");
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	Если Не (ПравоДоступа("Чтение", Метаданные.РегистрыСведений.СостоянияЗаказовПоставщикам)
			И ПравоДоступа("Изменение", Метаданные.Документы.ЗаявкаНаЗакупку)
	...
	ИмяФормыРабочееМестоЗаявкаНаЗакупку = "Документ.ЗаявкаНаЗакупку.Форма.ФормаСпискаДокументов";
	Если Запрос.Выполнить().Пустой() Тогда
		Возврат Новый ФорматированнаяСтрока(ТекстГиперссылки,,ЦветаСтиля.НезаполненноеПолеТаблицы,,
			ИмяФормыРабочееМестоЗаявкаНаЗакупку);
	Иначе
