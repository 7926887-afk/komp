	...
	Если ЗначениеЗаполнено(Партнер) Тогда
		...
		Если УсловияЗакупокПоУмолчанию <> Неопределено Тогда
			...
			Если Соглашение <> УсловияЗакупокПоУмолчанию.Соглашение
				...
				ПараметрыЗаполнения = Документы.ЗаявкаНаЗакупку.ПараметрыЗаполненияНалогообложенияНДСЗакупки(ЭтотОбъект);
				УчетНДСУП.ЗаполнитьНалогообложениеНДСЗакупки(НалогообложениеНДС, ПараметрыЗаполнения);
				
				ПараметрыЗаполнения = Документы.ЗаявкаНаЗакупку.ПараметрыЗаполненияВидаДеятельностиНДС(ЭтотОбъект);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ПараметрыВыбораСтатейИАналитик = Документы.ЗаявкаНаЗакупку.ПараметрыВыбораСтатейИАналитик(ХозяйственнаяОперация);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Документы.ЗаявкаНаЗакупку.ЗаполнитьИменаРеквизитовПоХозяйственнойОперации(
		ХозяйственнаяОперация,
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ОбщегоНазначенияУТ.ЗаполнитьИдентификаторыДокумента(ЭтотОбъект, Метаданные.Документы.ЗаявкаНаЗакупку.ТабличныеЧасти.Товары.Имя);
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//++ КОМПИТ
	_ОбработкаЗакупкиСервер.ПроверитьОплатуПоЗаказуКлиента(ДокументОснование, Отказ);
	
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ЭтотОбъект.ОбработкаЗаказаКлиента Тогда
	
		Если Значениезаполнено(ЭтотОбъект.КлючУникальности) Тогда
			КлючУникальности = ЭтотОбъект.КлючУникальности;
		Иначе
			КлючУникальности = Новый УникальныйИдентификатор;
		КонецЕсли;
		
		Если НЕ ЭтотОбъект.ОбработкаЗаказаПоставщика Тогда
			_ОбработкаЗакупкиСервер.СформироватьЗаписиРегистра_ЗаказЗакупка(ЭтотОбъект, КлючУникальности);
			ЭтотОбъект.КлючУникальности = КлючУникальности; 
			Если ТипЗнч(ЭтотОбъект.ДокументОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
				_ОбработкаЗакупкиСервер.ОбработкаЗаказаКлиентаПоЗаявкеНаЗакупку(ЭтотОбъект, КлючУникальности);
			ИначеЕсли ТипЗнч(ЭтотОбъект.ДокументОснование) = Тип("ДокументСсылка.ЗаказНаВнутреннееПотребление") Тогда
				_ОбработкаЗакупкиСервер.ОбработкаВнутреннегоЗаказаПоЗаявкеНаЗакупку(ЭтотОбъект, КлючУникальности);
			КонецЕсли;
			ЗаполнитьНазначение();
		КонецЕсли; 
		ЭтотОбъект.ОбработкаЗаказаКлиента    = Истина;
		ЭтотОбъект.ОбработкаЗаказаПоставщика = Ложь;
		ЭтотОбъект.Записать(РежимЗаписиДокумента.Запись);
	КонецЕсли;	//НЕ ЭтотОбъект.ОбработкаЗаказаКлиента
	Возврат;
	//--
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	//++ КОМПИТ
	Если ЭтотОбъект.ОбработкаЗаказаКлиента Тогда
		
		Если (Значениезаполнено(ЭтотОбъект.КлючУникальности)) И (НЕ ЭтотОбъект.ОбработкаЗаказаПоставщика) Тогда
			КлючУникальности = ЭтотОбъект.КлючУникальности;
			Если ТипЗнч(ЭтотОбъект.ДокументОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
				_ОбработкаЗакупкиСервер.ОбработкаЗаказаКлиентаПоЗаявкеНаЗакупку(ЭтотОбъект, КлючУникальности, Истина);
			ИначеЕсли ТипЗнч(ЭтотОбъект.ДокументОснование) = Тип("ДокументСсылка.ЗаказНаВнутреннееПотребление") Тогда
				_ОбработкаЗакупкиСервер.ОбработкаВнутреннегоЗаказаПоЗаявкеНаЗакупку(ЭтотОбъект, КлючУникальности, Истина);
			КонецЕсли;
			_ОбработкаЗакупкиСервер.СформироватьЗаписиРегистра_ЗаказЗакупка(ЭтотОбъект, КлючУникальности, Истина);
		КонецЕсли;
		ЭтотОбъект.ОбработкаЗаказаПоставщика = Ложь;
		ЭтотОбъект.ОбработкаЗаказаКлиента    = Ложь;
		ЭтотОбъект.Записать(РежимЗаписиДокумента.Запись);
	
	КонецЕсли;	//ЭтотОбъект.ОбработкаЗаказаКлиента
	Возврат;
	//--
...
Процедура ЗаполнитьДокументПоОтбору(Знач ДанныеЗаполнения)
	...
	Если ДанныеЗаполнения.Свойство("Товары") Тогда //Заполнение из обработки "Обеспечение потребностей".
		...
		Иначе
			...
			ХозяйственнаяОперация = Метаданные.Документы.ЗаявкаНаЗакупку.Реквизиты.ХозяйственнаяОперация.ЗначениеЗаполнения;
...
Процедура ЗаполнитьДокументНаОснованииЗаказаКлиента(Знач ДанныеЗаполнения)
	...
	//++ КОМПИТ
	Документы.ЗаказКлиента.Кт_ПроверитьВозможностьВводаНаОсновании(
		ЗаказКлиента,
	...
	ТаблицаТовары = _ОбработкаЗакупкиСервер.ТаблицаОстатковКЗаказу(ЗаказКлиента);
...
Процедура ЗаполнитьДокументНаОснованииЗаказаНаВнутреннееПотребление(Знач ДанныеЗаполнения)
	...
	//ЗаполнитьТоварыИзОснования(ДанныеЗаполнения.АдресТовары);
	ТаблицаТовары = _ОбработкаЗакупкиСервер.ТаблицаОстатковВнутрЗаказаКЗаказу(ЗаказНаПотребление);
	Товары.Загрузить(ТаблицаТовары);
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
	СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
	//ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(КэшированныеЗначения.ОбработанныеСтроки,СтруктураДействий, Неопределено);
...
Функция СкорректироватьСтрокиЗаказа(СтруктураПараметров) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.ЗаказыПоставщикам.Обороты(&НачПериод,&КонПериод,, ЗаявкаНаЗакупку = &ЗаявкаНаЗакупку) КАК ТоварыКПоступлению
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(Остатки.ЗаявкаНаЗакупку КАК Документ.ЗаявкаНаЗакупку).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден)
	|			ТОГДА Остатки.КОформлениюОстаток
	|		ИНАЧЕ Остатки.ЗаказаноОстаток
	|	КОНЕЦ КАК КОформлениюОстаток	
	|ПОМЕСТИТЬ ВтОстатки
	|ИЗ
	|	РегистрНакопления.ЗаказыПоставщикам.Остатки(, ЗаявкаНаЗакупку = &ЗаявкаНаЗакупку) КАК Остатки
	|ГДЕ
	...
	|	ТаблицаТовары КАК ТаблицаТовары
	|ГДЕ
	|	ТаблицаТовары.ЗаявкаНаЗакупку.Статус НЕ В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден),
	...
	Запрос.УстановитьПараметр("ЗаявкаНаЗакупку", Ссылка);
	...
	ГраницыОборотов = ОбщегоНазначенияУТ.ГраницыОборотовРегистра("ЗаказыПоставщикам",
																"ЗаявкаНаЗакупку = &ЗаявкаНаЗакупку",
																Запрос.Параметры);
				...
				Если СтрокиТоваров.Количество() > 0 Тогда
					...
					Для каждого Строка Из СтрокиТоваров Цикл
						...
						Иначе
							...
							КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
							ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(Строка, СтруктураДействий, КэшированныеЗначения);
...
//
// Параметры:
// 		ЗаказОбъект - ДокументОбъект.ЗаявкаНаЗакупку - Корректируемый заказ поставщику.
	...
	Для Каждого СтрокаОтклонений Из ОтклоненияЦен Цикл
		...
		Если НЕ СтрокаОтклонений.ЦенаИзменилась И СтрокаОтклонений.СуммаИзменилась Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				КонецЕсли;
				ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НайденнаяСтрока, СтруктураДействийИзмененияСумм, Неопределено);
		...
		Иначе
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				Иначе
					...
					НайденнаяСтрока.ПроцентРучнойСкидки = 0;
				КонецЕсли;
				ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НайденнаяСтрока, СтруктураДействийИзмененияЦен, Неопределено);
...
Функция ОтменитьНепоставленныеСтроки(ПричинаОтмены, Знач ПроверятьОстатки = Ложь) Экспорт
	...
	|ИЗ
	|	ТаблицаТовары КАК ТаблицаТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыПоставщикам.ОстаткиИОбороты(,,,, ЗаявкаНаЗакупку = &ЗаявкаНаЗакупку) КАК Остатки
	...
	|	ТаблицаТовары КАК ТаблицаТовары
	|ГДЕ
	|	ВЫРАЗИТЬ(ТаблицаТовары.ЗаявкаНаЗакупку КАК Документ.ЗаявкаНаЗакупку).Статус НЕ В (ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден),
	|							ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Закрыт))
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки;
	|
	...
	|		,,,,
	|		ДокументПоступления = &ЗаявкаНаЗакупку) КАК ТоварыКПоступлениюОстатки");
	...
	РаспоряжениеЗаказ = Документы.ЗаявкаНаЗакупку.ЗаказЯвляетсяРаспоряжениемНаПриемкуТоваров(Ссылка);
...
Процедура ЗаполнитьЦеныПоЗаказуПринципала()
	...
	СтруктураДействийПоПересчетуРучнойСкидки.Вставить("ПересчитатьЦенуСкидкуПоСуммеВЗакупках", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруПересчетаЦеныСкидкиВЗакупкахВТЧ(Товары, ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПриемНаКомиссию")));
	...
	Для каждого СтрокаТовара Из Товары Цикл
		...
		Если ПересчитатьСуммы Тогда
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТовара, СтруктураДействий, КэшированныеЗначения);
		Иначе
			СтрокаТовара.Сумма = СуммаТовара;
			СтрокаТовара.СуммаСНДС = СуммаТовараСНДС;
			СтрокаТовара.СуммаНДС = СуммаТовараНДС;
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТовара, СтруктураДействийПоПересчетуРучнойСкидки, КэшированныеЗначения);
		КонецЕсли;
...
Процедура ЗаполнитьУИД()
	...
	Для Каждого ТекСтрока Из Товары Цикл
		Если НЕ ЗначениеЗаполнено(ТекСтрока._УИДЗакупка) Тогда
			ТекСтрока._УИДЗакупка = Новый УникальныйИдентификатор();
		КонецЕсли;
...
Процедура ПроверитьУИД()
	...
	Для Каждого ТекСтрока Из Товары Цикл
		СтрПоиска = Новый Структура("_УИДЗакупка", ТекСтрока._УИДЗакупка);
...
Процедура ЗаменитьУИД(МасСтрок)
	...
	Для к = 1 По МасСтрок.Количество() - 1 Цикл
		Если Товары[МасСтрок[0].НомерСтроки - 1]._УИДЗакупка = Товары[МасСтрок[к].НомерСтроки - 1]._УИДЗакупка Тогда
			Товары[МасСтрок[к].НомерСтроки - 1]._УИДЗакупка = Новый УникальныйИдентификатор();
		КонецЕсли;
	КонецЦикла;		// к = 1
...
Процедура ЗаполнитьНазначение()
	...
	НазначениеСсылка = Справочники.Назначения.НайтиПоРеквизиту("Заказ", ЭтотОбъект.ДокументОснование); 
	Если НЕ НазначениеСсылка.Пустая() Тогда 
		ТабТовары = ЭтотОбъект.Товары;
		Для Каждого СтрокаТовары Из ТабТовары Цикл
			СтрокаТовары.Назначение = НазначениеСсылка;
		КонецЦикла;		// СтрокаТовары Из Товары
	КонецЕсли;
	
КонецПроцедуры	// ЗаполнитьНазначение()
//== Сухарев Б.В
