...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ КАВ
	//Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ЗаказНаВнутреннееПотребление") Тогда
	//	Если Объект.ДокументОснование.Согласован = Ложь Тогда
	//		Отказ = Истина;
	//	КонецЕсли;
	//КонецЕсли;
	//--
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Объект.СтатусЗаявкиНаЗакупку = Перечисления.СтатусыЗаявокНаЗакупку.ПереданаЗакупке;
	КонецЕсли;
	//--
	
	//++ Максимов
	Если НЕ РольДоступна("АдминистраторСистемы") Тогда
		Элементы.ГруппаСтраницы.ПодчиненныеЭлементы.ГруппаДополнительно.Видимость = Ложь;
	КонецЕсли;
	//--
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		//Возврат;
		Если НЕ Объект.ДокументОснование = _ДоработкиСлужебный.ПолучитьПустуюСсылку(ТипЗнч(Объект.ДокументОснование)) Тогда
			Парам  = Новый Структура("Ключ", Объект.ДокументОснование);
			мФорма = ПолучитьФорму("Документ.ЗаказКлиента.ФормаОбъекта", Парам); 
			Если мФорма.Открыта() Тогда
				Попытка
					мФорма.Закрыть(0);
				Исключение
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Заказ '") + Объект.ДокументОснование + НСтр("ru=' открыт другим пользователем или фоновым заданием'"));
				КонецПопытки;
			КонецЕсли;
		КонецЕсли; 
		//== Сухарев Б.В.
	...
	//++ КАВ начало
	Отказ = ПроверкаСоставаТоваров();
	//-- КАВ конец
	
	//++ Сухарев Б.В. 01.12.2018
	Отказ = Отказ И ПроверкаКоличестваТоваров();
	//== Сухарев Б.В.
	
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Отказ = Отказ ИЛИ НЕ ПроверитьОплатуПоОснованиюНаСервере(Объект.ДокументОснование);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ПроверитьОплатуПоОснованиюНаСервере(ДокументОснование)
...
//++ КАВ начало
Функция ПроверкаСоставаТоваров()
	...
	Иначе
		...
		Для Каждого Стр Из Объект.ДокументОснование.Товары Цикл
			//++ НВА 2025-05-15
			ТЗКомплекта = _ОбработкаЗакупкиСервер.ПолучитьНоборКомплекта(Стр.Номенклатура, Стр.Характеристика);
			Если ТЗКомплекта.Количество() > 0 Тогда
				Для Каждого СтрК Из ТЗКомплекта Цикл
					СтрТЗ = ТЗ.Добавить();
					СтрТЗ.Номенклатура = СтрК.Номенклатура;
					СтрТЗ.Количество = Стр.Количество * СтрК.Количество;
					СтрТЗ.Признак = 1;
				КонецЦикла;
			Иначе
				СтрТЗ = ТЗ.Добавить();
		...
		ТЗ.Свернуть("Номенклатура, Количество", "Признак");
		Для Каждого Стр Из ТЗ Цикл
			Если Стр.Признак = 2 Тогда
				Сообщить("В заявке и заказе клиента различается состав товаров.");
				Отказ = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
//++ Сухарев Б.В. 01.12.2018
&НаСервере
Функция ПроверкаКоличестваТоваров()
	...
	Иначе
		...
		Для Каждого Стр Из Объект.ДокументОснование.Товары Цикл
			Если Стр._ЗаявкаНаЗакупку <> Объект.Ссылка Тогда
				Продолжить;
			КонецЕсли;
			СтрТЗ = ТЗ.Добавить();
			ЗаполнитьЗначенияСвойств(СтрТЗ, Стр);
			СтрТЗ.КоличествоУпаковок = -1 * СтрТЗ.КоличествоУпаковок; 
			СтрТЗ.Количество		 = -1 * СтрТЗ.Количество; 
			ТЗКомплекта = _ОбработкаЗакупкиСервер.ПолучитьНоборКомплекта(Стр.Номенклатура, Стр.Характеристика);
			Если ТЗКомплекта.Количество() > 0 Тогда
				КолВЗаказеУп = СтрТЗ.КоличествоУпаковок;
				КолВЗаказе   = СтрТЗ.Количество;
				СтрТЗ.КоличествоУпаковок = 0;
				Для Каждого СтрКомплекта Из ТЗКомплекта Цикл
					ДопСтрТЗ = ТЗ.Добавить();
					ЗаполнитьЗначенияСвойств(ДопСтрТЗ, СтрКомплекта);
					ДопСтрТЗ.КоличествоУпаковок = ДопСтрТЗ.КоличествоУпаковок * КолВЗаказеУп; 
					ДопСтрТЗ.Количество			= ДопСтрТЗ.Количество * КолВЗаказе; 
		...
		ТЗ.Свернуть("Номенклатура", "КоличествоУпаковок, Количество");
//		Если НЕ ТЗ.Итог("КоличествоУпаковок") = 0 Тогда
		Если НЕ ТЗ.Итог("Количество") = 0 Тогда
			Для Каждого Стр Из ТЗ Цикл
				Если Стр.КоличествоУпаковок > 0 Тогда
					ТекстСообщения  = Стр.Номенклатура.Наименование + НСтр("ru='. Вы не можете заказать больше чем в Заказе клиента'");
...
КонецФункции // ПроверкаКоличестваТоваров()
//== Сухарев Б.В.
...
Процедура ТоварыУпаковкаПриИзменении(Элемент)
	...
	//++ КОМПИТ
	Если НЕ ЗначениеЗаполнено(_СтараяУпаковка) Тогда
		Если ТекущаяСтрока.Упаковка.Пустая() Тогда
			_СтараяУпаковка = ТекущаяСтрока.Номенклатура;
		Иначе
			_СтараяУпаковка = ТекущаяСтрока.Упаковка;
		КонецЕсли;
	КонецЕсли;
	//--
...
Процедура КоличествоУпаковокПриИзменении(Элемент)
	...
	Если Объект.Проведен Тогда
		ТекстСообщения  = НСтр("ru='Для изменения количества в заявке отмените проведение документа'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		//КоличествоУпаковок = ЭтотОбъект.ТоварыКешТекущейСтроки.КоличествоУпаковок;
		ОбновитьФорму();
		Возврат;
	КонецЕсли;	// Объект.Проведен 
	//== Сухарев Б.В
	...
	Если ТекущаяСтрока._НоменклатураКомплект = _ДоработкиСлужебный.ПолучитьПустуюНоменклатуру() 
		И ТекущаяСтрока._КоличествоУпаковокКомплекта = 0 Тогда
		
		ПакетнаяОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
		
		РассчитатьИтоговыеПоказателиЗаказа(ЭтаФорма);
	Иначе
		ТекстСообщения  = НСтр("ru='Эта номенклатура входит в комплект'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		ОбновитьФорму();
		Возврат;
...
Процедура ТоварыПриАктивизацииЯчейки(Элемент)
	...
	//++ КОМПИТ
	ИначеЕсли Элемент.ТекущийЭлемент.Имя = "ТоварыУпаковка" Тогда
		ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
...
Процедура НалогообложениеНДСПриИзмененииСервер(КэшированныеЗначения)
	...
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("СкорректироватьСтавкуНДС", ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект, Ложь, Объект.ВернутьМногооборотнуюТару));
	...
	СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
	СтруктураДействий.Вставить("ПересчитатьСуммуСНДС", СтруктураПересчетаСуммы);
	ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(КэшированныеЗначенияСлужебные.ОбработанныеСтроки, СтруктураДействий, КэшированныеЗначения);
...
Процедура ЖелаемаяДатаПоступленияПриИзмененииСервер()
	ВзаиморасчетыСервер.ПриИзмененииПараметровМеханизма(ЭтаФорма);
	//++ Сухарев Б.В. 04.01.2019
	мДатаПоступления = НачалоДня(Объект.ЖелаемаяДатаПоступления);
	Для Каждого ТекСтрока Из Объект.Товары Цикл
		ТекСтрока.ДатаПоступления = мДатаПоступления;
...
Функция СкорректироватьЦеныПоПоступлениямСервер()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			КонецЕсли;
			ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НайденныеСтроки[0], СтруктураДействий, Неопределено);
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		//++ Сухарев Б.В. 01.02.2019
		ТекущаяСтрока._УИДЗакупка = Новый УникальныйИдентификатор();
		//== Сухарев Б.В
		
		Если Объект.ПоступлениеОднойДатой Тогда
			//++ Сухарев Б.В. 01.02.2019
			//ТекущаяСтрока.ДатаПоступления = Объект.ДатаПоступления;
			ТекущаяСтрока.ДатаПоступления = ?(ЗначениеЗаполнено(Объект.ДатаПоступления), Объект.ДатаПоступления, ТекущаяДата());
			//== Сухарев Б.В
		КонецЕсли;
...
Процедура ЗаполнитьТоварыИзВременногоХранилищаСервер(АдресВоВременномХранилище)
	...
	Для каждого ТекСтрока Из ТаблицаТоваров Цикл
		...
		КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаКоллекции, СтруктураДействий, КэшированныеЗначения);
...
Процедура СоздатьЗаказПоставщику(Команда)
	...
	Отказ = Ложь;
	Если Отказ ИЛИ НЕ Объект.Проведен Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ОткрытьФорму("Документ.ЗаказПоставщику.ФормаОбъекта", Новый Структура("Основание", Объект.Ссылка)); //YuS с документа основания на ссылку
	КонецЕсли;
	ЭтаФорма.Закрыть();
...
Процедура ПолучитьДоступКПолям()
	...
	Если _ДоработкиСлужебный.ПолучитьДоступКполямДокумента() Тогда
		ЭтаФорма.Элементы.ТоварыЗаказано.ТолькоПросмотр			= Ложь;
		ЭтаФорма.Элементы.ТоварыЗаказПоставщику.ТолькоПросмотр	= Ложь;
	Иначе
		ЭтаФорма.Элементы.ТоварыЗаказано.ТолькоПросмотр			= Истина;
		ЭтаФорма.Элементы.ТоварыЗаказПоставщику.ТолькоПросмотр	= Истина;
	КонецЕсли;
