...
Процедура СформироватьВременнуюТаблицуТоваровДляПечати_Дополнительный(МенеджерВременныхТаблиц, МассивОснований, ПараметрыПечати)
	...
	Для Каждого ТипОснования Из ТипыОснований Цикл
		...
		Если ИмяОбъекта = "АктВыполненныхРабот"
				...
				Или ИмяОбъекта = "РеализацияТоваровУслуг" Тогда
		...
		КонецЕсли;
		ТекстОбщейТаблицыТовары = СтрЗаменить(ТекстОбщейТаблицыТовары, "//ДанныеНоменклатурыПартнера", ТекстЗамены);
	...
	Кт_ОбщегоНазначенияСервер.ПодменитьСодержаниеПоНоменклатуреПартнераВТаблицеТоваров(МенеджерВременныхТаблиц);
	
	ПараметрыПечати.ВыводитьНаборы = ЕстьНаборы;
	
КонецПроцедуры

&После("СформироватьВременнуюТаблицуТоваровДляПечати")
Процедура Кт_ПослеСформироватьВременнуюТаблицуТоваровДляПечати(МенеджерВременныхТаблиц, МассивОснований, ПараметрыПечати)
