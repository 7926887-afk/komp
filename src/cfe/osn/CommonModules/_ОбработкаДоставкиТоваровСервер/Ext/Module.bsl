...
// Единая дата отгрузки заявки: шапочная ДатаОтгрузки, иначе ЖелаемаяДатаОтгрузки, иначе текущая дата сеанса.
&НаСервере
Функция ДатаОтгрузкиЗаявки(Заявка) Экспорт
	
	Если ЗначениеЗаполнено(Заявка.ДатаОтгрузки) Тогда
		Возврат НачалоДня(Заявка.ДатаОтгрузки);
	ИначеЕсли ЗначениеЗаполнено(Заявка.ЖелаемаяДатаОтгрузки) Тогда
		Возврат НачалоДня(Заявка.ЖелаемаяДатаОтгрузки);
	Иначе
...
Функция ОбработатьДоставкуТовары(Объект, ОбъектЗаказ)
	...
	ДатаОтгрузки	= ДатаОтгрузкиЗаявки(Объект);
	...
	Для Каждого СтрокаДоставки Из ТоварыЗаявки Цикл
		...
		Если НЕ СтрокиЗаказа.Количество() = 0 Тогда
			Для Каждого СтрокаЗаказа Из СтрокиЗаказа Цикл
				СтрокаЗаказа.ДатаОтгрузки = ДатаОтгрузки;
...
Процедура ЗаписатьДоставкуЗаказаКлиента(Объект, ОбъектЗаказ)
	...
	Иначе 
		...
		ОбъектЗаказ.ДатаОтгрузки						= ДатаОтгрузкиЗаявки(Объект);
					...
					Если НЕ Удаление  Тогда	
						...
						//Если ( Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.КОбеспечению ) ИЛИ ( Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно ) Тогда
							...
							Иначе			// Строка.Количество > ТекСтрока.Количество
								...
								РазбитьСтрокуТЧ(ТоварыЗаказа, Строка, КолНаОтгрузку, ПараметрыОбработки, ДатаОтгрузки);
								КолНаОтгрузку = 0;
					...
					Если НЕ Удаление  Тогда
						...
						Если Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.КОбеспечению Тогда
							Если Строка.КоличествоУпаковок <= КолНаОтгрузку Тогда
								...
								КонецЕсли;
								Строка.ДатаОтгрузки = ДатаОтгрузки;
								КолНаОтгрузку = КолНаОтгрузку - Строка.КоличествоУпаковок;
								Если КолНаОтгрузку < 0 Тогда
									КолНаОтгрузку = 0;
								КонецЕсли;
...
Процедура ПерезаполнитьТабличныеЧастиРО(ЗаявкаНаДоставку, РасходныйОрдерОбъект)
	...
	|	ТоварыЗаявки.Характеристика КАК Характеристика,
	|	ТоварыЗаявки.Серия КАК Серия,
	|	ТоварыЗаявки.Количество КАК Количество,
	|	ТоварыЗаявки.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ТоварыЗаявки._УИДОтгрузка КАК _УИДОтгрузка
	|ПОМЕСТИТЬ ТоварыЗаявки
	|ИЗ
	|	Документ._ЗаявкаНаДоставку.Товары КАК ТоварыЗаявки
	|ГДЕ
	|	ТоварыЗаявки.Ссылка = &ЗаявкаНаДоставку
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТоварыЗаявки.Количество КАК Количество,
	|	ТоварыЗаявки.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ЗаказТоварыТ.Ссылка КАК Распоряжение,
	|	ЗНАЧЕНИЕ(Перечисление.ДействияСоСтрокамиОрдеровНаОтгрузку.Отобрать) КАК Действие
	|ИЗ
	|	ТоварыЗаявки КАК ТоварыЗаявки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.Товары КАК ЗаказТоварыТ
	|		ПО (ЗначениеЗаполнено(ТоварыЗаявки._УИДОтгрузка)
	|				И ТоварыЗаявки._УИДОтгрузка = ЗаказТоварыТ._УИДОтгрузка)
	|			ИЛИ (ТоварыЗаявки.Номенклатура = ЗаказТоварыТ.Номенклатура
	|				И ТоварыЗаявки.Характеристика = ЗаказТоварыТ.Характеристика
	|				И ТоварыЗаявки.Серия = ЗаказТоварыТ.Серия
	|				И (НЕ ЗначениеЗаполнено(ТоварыЗаявки._УИДОтгрузка)
	|					ИЛИ ЗаказТоварыТ._Отгрузка))
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК НазначенияТ
	|		ПО ЗаказТоварыТ.Ссылка = НазначенияТ.Заказ
	|			И НЕ НазначенияТ.ПометкаУдаления
	|ГДЕ
	|	ЗаказТоварыТ.Ссылка В(&Заказы)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТоварыЗаявки.Номенклатура,
	|	ТоварыЗаявки.Характеристика,
	|	НазначенияТ.Ссылка,
	|	ТоварыЗаявки.Серия,
	|	ТоварыЗаявки.Количество,
	|	ТоварыЗаявки.КоличествоУпаковок,
	|	ЗаказТоварыТ.Ссылка,
	|	ЗНАЧЕНИЕ(Перечисление.ДействияСоСтрокамиОрдеровНаОтгрузку.Отобрать)
	|ИЗ
	|	ТоварыЗаявки КАК ТоварыЗаявки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаВнутреннееПотребление.Товары КАК ЗаказТоварыТ
	|		ПО (ЗначениеЗаполнено(ТоварыЗаявки._УИДОтгрузка)
	|				И ТоварыЗаявки._УИДОтгрузка = ЗаказТоварыТ._УИДОтгрузка)
	|			ИЛИ (ТоварыЗаявки.Номенклатура = ЗаказТоварыТ.Номенклатура
	|				И ТоварыЗаявки.Характеристика = ЗаказТоварыТ.Характеристика
...
Процедура ОформитьРасходныеОрдераПоДоставкеНаСервере(МассивРаспоряжений, ЗаявкаНаДоставку, НовыйОрдер = Истина) Экспорт
	...
	|	ЕСТЬNULL(РеквизитыРаспоряжения.ДатаДокументаИБ, &ДатаОтгрузкиЗаявки) КАК Дата
	...
	Запрос.УстановитьПараметр("ДатаОтгрузкиЗаявки", ДатаОтгрузкиЗаявки(ЗаявкаНаДоставку));
			...
			Пока Выборка.СледующийПоЗначениюПоля("Получатель") Цикл	
				...
				Для Каждого ОформленныеОрдераСтрока Из СтруктураЗадания.ОформленныеОрдера Цикл
					Если НовыйОрдер Тогда
						...
						РасходныйОрдерОбъект.ДатаОтгрузки	   = ДатаОтгрузкиЗаявки(ЗаявкаНаДоставку);
