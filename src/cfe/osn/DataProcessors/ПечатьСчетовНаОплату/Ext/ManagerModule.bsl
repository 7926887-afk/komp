...
Функция СформироватьПечатнуюФормуСчетНаОплатуДополнительный(СтруктураТипов, ОбъектыПечати, ПараметрыПечати, КомплектыПечати = Неопределено) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		//** 
		//Если ПараметрыСеанса.ДопСчетНаОплату_Аванс = Истина Тогда
		//	ДанныеДляПечати = МенеджерОбъекта.ПолучитьДанныеДляПечатнойФормыСчетаНаОплатуДополнительный_Аванс(ПараметрыПечати, СтруктураОбъектов.Значение);
		//	ЗаполнитьТабличныйДокументСчетаНаОплатуДополнительный_Аванс(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, КомплектыПечати, ПараметрыПечати);
		//Иначе
		//	ДанныеДляПечати = МенеджерОбъекта.ПолучитьДанныеДляПечатнойФормыСчетаНаОплатуДополнительный_НеАванс(ПараметрыПечати, СтруктураОбъектов.Значение);
		//	ЗаполнитьТабличныйДокументСчетаНаОплату(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати);	//**  , КомплектыПечати, ПараметрыПечати);
		//КонецЕсли;
		//
...
Процедура ЗаполнитьТабличныйДокументСчетаНаОплатуДополнительный_Аванс(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, КомплектыПечати, ПараметрыПечати)
	...
	Результат= ФормированиеПечатныхФорм.ДополнительнаяКолонкаПечатныхФормДокументов();
	СтруктураИмяДопКолонки = Новый Структура("ИмяКолонкиКодов", Результат.ИмяКолонки);
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ТаблицаЭтапыОплаты.Количество() > 1 Тогда
			Массив = Новый Массив;
			Массив.Добавить(ТаблицаЭтапыОплаты[0]);
			ТаблицаЭтапыОплаты.Очистить();
			ТаблицаЭтапыОплаты.Добавить(Массив[0]);
		КонецЕсли;
		...
		а = 1;
		Если а = 1 ИЛИ ДанныеПечати.ЧастичнаяОплата ИЛИ ТипЗнч(ДанныеПечати.ДокументОснование) = Тип("ДокументСсылка.ОтчетКомитенту") Тогда
			...
			ОбластьСтрокаТаблицы.Параметры.НазначениеПлатежа = "предоплата " + ПараметрыСеанса.ДопСчетНаОплату_Процент + "% в размере " + ПараметрыСеанса.ДопСчетНаОплату_Сумма;
...
Процедура Кт_ЗаполнитьТабличныйДокументСчетаНаОплату(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		#Вставка
		//РИВ +++ 02.12.25
		ИНН = ДанныеПечати.Организация.ИНН; 				
		Флаг = Ложь;
		
		Если ИНН = "7743342864" Тогда 
			ОбластьИнф = Макет.ПолучитьОбласть("ОбластьИнф_ОФИСОФТ");
			Флаг = Истина;
		ИначеЕсли ИНН = "7743342977" Тогда
			ОбластьИнф = Макет.ПолучитьОбласть("ОбластьИнф_ТриТехноложди");
			Флаг = Истина;
		ИначеЕсли ИНН = "7718302970" Тогда 
			ОбластьИнф = Макет.ПолучитьОбласть("ОбластьИнф_КОМПИТ");
			Флаг = Истина;
		ИначеЕсли ИНН = "7743115854" Тогда
			ОбластьИнф = Макет.ПолучитьОбласть("ОбластьИнф_ИТТрейд");
			Флаг = Истина;
		ИначеЕсли ИНН = "7743137872" Тогда		
			ОбластьИнф = Макет.ПолучитьОбласть("ОбластьИнф_ПБИГрупп");
			Флаг = Истина;
		КонецЕсли; 
		
		Если Флаг Тогда
			ОбластьЗаголовокДопИнф = Макет.ПолучитьОбласть("ОбластьЗаголовокДопИнф");
			ТабличныйДокумент.Вывести(ОбластьЗаголовокДопИнф);
			ТабличныйДокумент.Вывести(ОбластьИнф);
		КонецЕсли;
		//РИВ --- 02.12.25
		#КонецВставки	
...
Процедура Кт_ЗаполнитьРеквизитыПодвалаСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, СоответствиеСтавокНДС, ПараметрыПечати)
...
Процедура Кт_Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	//КАВ начало
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетНаОплатуДополнительный") Тогда
		СтруктураТипов = ОбщегоНазначенияУТ.СоответствиеМассивовПоТипамОбъектов(МассивОбъектов);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
		КоллекцияПечатныхФорм,
		"СчетНаОплатуДополнительный",
		НСтр("ru='Счет на оплату'"),
		СформироватьПечатнуюФормуСчетНаОплатуДополнительный(СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
	КонецЕсли;
	//КАВ конец
...
Процедура Кт_ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, ТаблицаТовары)
	...
	Иначе
		...
		Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") Тогда //
			Если ( ДанныеПечати.Ссылка.Кт_Контракт ) Тогда
				СтруктураДанныхШапки.Вставить("НазначениеПлатежа", "Оплата по договору № " + ДанныеПечати.Ссылка.Кт_КонтрактНомер + " от " + Формат( ДанныеПечати.Ссылка.Кт_КонтрактДата, "ДЛФ=Д") + " г." );
			Иначе
				СтруктураДанныхШапки.Вставить("НазначениеПлатежа", "Оплата по " + СокрЛП(ДанныеПечати.Ссылка.Договор.Наименование)); 
			КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ДанныеПечати.Контрагент) Тогда
		Если ЗначениеЗаполнено(ДанныеПечати.Ссылка) Тогда
			ДанныеПечати.Контрагент = Справочники.Контрагенты.НайтиПоНаименованию(ДанныеПечати.Ссылка.Партнер.Наименование);
		КонецЕсли;	
	КонецЕсли;
	//**
	#КонецВставки
