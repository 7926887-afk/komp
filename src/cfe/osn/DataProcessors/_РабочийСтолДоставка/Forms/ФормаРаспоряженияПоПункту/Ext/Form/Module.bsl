...
#Область РасчетТоваров
	...
	Иначе
		...
		|ПОМЕСТИТЬ ТоварыРаспоряжения
		|ИЗ
		|	РегистрНакопления.ТоварыКОтгрузке.Обороты(, , , ДокументОтгрузки В (&Ссылки)) КАК Т
		|ГДЕ
		|	Т.КОтгрузкеПриход > 0"
	КонецЕсли;
	
	ТекстЗапроса = ТекстЗапроса + ИнтеграцияИС.РазделительЗапросовВПакете() +
	"ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВлТоварыКДоставке.Распоряжение КАК Распоряжение,
	|	ВлТоварыКДоставке.Склад КАК Склад,
	|	ВлТоварыКДоставке.ПолучательОтправитель КАК ПолучательОтправитель,
	|	МИНИМУМ(ВлТоварыКДоставке.ВсеТовары) КАК ВсеТовары,
	|	ВлТоварыКДоставке.Номенклатура КАК Номенклатура,
	|	ВлТоварыКДоставке.Характеристика КАК Характеристика,
	|	ВлТоварыКДоставке.Назначение КАК Назначение,
	|	ВлТоварыКДоставке.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ВлТоварыКДоставке.Серия КАК Серия,
